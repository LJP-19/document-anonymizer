...
def check_tree_is_consistent() -> None:
    ...
    """
    # A missing third-party package is an environment problem, not a broken
    # tree. Only a failure inside the project's own modules means the files
    # disagree with each other, which is what this gate exists to catch.
    script = (
        ...
        "            print('DEPENDENCY_MISSING:' + missing)\n"
        "            sys.exit(3)\n"
        "        traceback.print_exc()\n"
        "        sys.exit(1)\n"
        "    except Exception:\n"
    ...
    if result.returncode == 3:
        missing = result.stdout.strip().split(":", 1)[-1]
        warn(f"cannot check file consistency here: '{missing}' is not installed")
        warn("GitHub checks it on all three platforms before building anything")
        return
    ...
    if result.returncode != 0:
        ...
        stop(
            ...
            + "\n\n  A file is referring to something another file was supposed to "
            "define.\n  This usually means an older copy of a file survived an update. "
            "Re-apply\n  the latest build over the project and try again."
        )
