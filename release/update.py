...
ZIP_VERSION = re.compile(r"document-anonymizer-v(\d+(?:\.\d+)*)", re.I)
...
def version_of_archive(path: Path) -> tuple[int, ...]:
    match = ZIP_VERSION.search(path.name)
    return version_tuple(match.group(1)) if match else ()


def find_zip(explicit: str | None) -> Path | None:
    """Pick the archive with the HIGHEST version, not the newest file date.

    Choosing by modification time meant an old download whose timestamp had
    been touched could win, and an entire earlier build would be copied over a
    newer project. Version order is the only ordering that means anything here;
    file date is a tiebreaker between identical versions.
    """
    if explicit:
    ...
    if not found:
        return None
    return max(found, key=lambda p: (version_of_archive(p), p.stat().st_mtime))
...
def extract(archive: Path) -> Path:
    ...
    # Flat archive: the project files sit at the root.
    if (tmp / "app" / "version.py").exists():
        return tmp
    # Wrapped archive: one folder containing the project.
    for root in sorted(p for p in tmp.iterdir() if p.is_dir()):
        if (root / "app" / "version.py").exists():
            return root
        for nested in sorted(q for q in root.iterdir() if q.is_dir()):
            if (nested / "app" / "version.py").exists():
                return nested
...
def main() -> int:
    ...
    if archive is None:
        ...
        if version_tuple(incoming) < version_tuple(before):
            shutil.rmtree(source.parent, ignore_errors=True)
            stop(
                f"that archive is v{incoming}, older than this project (v{before}).\n"
                "  Applying it would overwrite newer work with an earlier build.\n\n"
                f"  Delete the old {archive.name} from your downloads, or pass the\n"
                "  archive you want explicitly:\n"
                "      PUBLISH.bat \"C:\\path\\to\\document-anonymizer-vX.Y.Z.zip\""
            )
        if incoming == before:
