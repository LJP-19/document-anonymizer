...
## Which archive gets applied

The highest version wins, not the newest file. If several
`document-anonymizer-v*.zip` files are lying around, the launcher picks the
greatest version number and refuses outright to apply one older than the
project. To force a specific archive:

    PUBLISH.bat "C:\path\to\document-anonymizer-v0.7.3.zip"
