...
def _count_value_occurrences(haystack: str, needle: str) -> int:
    """Same word-boundary/currency-exclusion rules as `_contains_value`, but
    counts every genuine occurrence instead of stopping at the first one -
    needed to tell "zero survive" apart from "exactly the number that were
    correctly never redacted survive"."""
...
def _contains_value(haystack: str, needle: str) -> bool:
    ...
    """
    return _count_value_occurrences(haystack, needle) > 0


class PIILeakError(RuntimeError):
    """A critical check failed on the saved file: something the plan should
    have removed is still there, or a replacement is missing or wrong.

    `verify()` already performs this exact sweep - it re-opens the SAVED file
    and confirms every accepted original is gone (`accepted originals
    removed`) and every replacement is present and red. This is a thin
    exception-raising wrapper for a caller that wants exception-based flow
    control (a CLI, an API endpoint) rather than checking `report.passed`
    itself. It adds no new checking logic - the report already did the work.
    """


def raise_if_leaked(report: VerificationReport) -> VerificationReport:
    """Raise PIILeakError if any CRITICAL check failed. Returns the report
    unchanged otherwise, so this can wrap a call: `raise_if_leaked(verify(...))`.
    """
    critical_failures = [c for c in report.checks if not c.passed and c.critical]
    if critical_failures:
        detail = "; ".join(f"{c.name}: {c.detail}" for c in critical_failures)
        raise PIILeakError(f"{len(critical_failures)} critical check(s) failed: {detail}")
    return report


def verify(plan: TransformationPlan, output_path: str) -> VerificationReport:
    report = VerificationReport(output_path=output_path)
    _lock = PDF_LOCK.acquire()
...
def _check_originals_absent(report: VerificationReport, plan: TransformationPlan, out_norm: str) -> None:
    expected = plan.expected_residual_counts or {}
    survivors = sorted(
        {
            t.candidate_id
            for t in plan.targets
            if _count_value_occurrences(
                out_norm, t.original
            ) > expected.get(" ".join(t.original.split()).strip(".,;: ").lower(), 0)
        }
    )
    report.add(
...
def _check_replacements_present(report: VerificationReport, plan: TransformationPlan, out_norm: str) -> None:
    # A blackout has no replacement text by design.
    missing = [
        t.candidate_id
        for t in plan.targets
        if not t.blackout and not _contains_value(out_norm, t.replacement)
    ]
    report.add(
...
def _check_replacement_colour(report: VerificationReport, plan: TransformationPlan, out: pymupdf.Document) -> None:
    ...
    not_red = [
        t.candidate_id
        for t in plan.targets
        if not t.blackout and _normalize(t.replacement) not in joined
    ]
    report.add(
...
def _check_group_completeness(report: VerificationReport, plan: TransformationPlan, out_norm: str) -> None:
    ...
    for group_id, member_ids in plan.group_membership.items():
        ...
        if any(
            _count_value_occurrences(out_norm, t.original)
            > expected.get(" ".join(t.original.split()).strip(".,;: ").lower(), 0)
            for t in members
        ):
...
def _check_partial_redaction(report: VerificationReport, plan: TransformationPlan, out_norm: str) -> None:
    ...
    The expected-residual allowance below covers a specific, narrow case: a
    token surviving standalone is only excused when every one of its
    standalone occurrences is accounted for by an INTACT full-phrase
    occurrence of the entire original value that itself falls within the
    proven expected residual (the organizational-suffix guard in
    propagate() is the source of that residual - "Marcus Feldman
    Manufacturing Inc." legitimately keeps the whole name intact). A token
    appearing MORE often alone than the full phrase appears at all means at
    least one occurrence is NOT part of an intact phrase - exactly the
    "John redacted, Smith survives alone" failure this check exists to
    catch - and that always fails, regardless of any allowance.
    """
    expected = plan.expected_residual_counts or {}
    residual: list[str] = []
    for t in plan.targets:
        original_key = " ".join(t.original.split()).strip(".,;: ").lower()
        allowance = expected.get(original_key, 0)
        full_phrase_count = _count_value_occurrences(out_norm, t.original) if allowance else 0
        tokens = [tok.strip(".,;:()") for tok in re.split(r"[\s,]+", t.original.strip())]
        ...
        for token in tokens:
            ...
            if token.lower() in _normalize(t.replacement):
                continue
            count = _count_value_occurrences(out_norm, token)
            # Every standalone appearance of this token must be explained by
            # an intact full-phrase occurrence within the proven allowance -
            # not merely "some allowance exists somewhere".
            if count > min(allowance, full_phrase_count):
...
def _check_group_residue(report: VerificationReport, plan: TransformationPlan, out_norm: str) -> None:
    ...
    line and the rest survived, so this checks the ORIGINAL LINE text. Same
    intact-phrase reasoning as `_check_partial_redaction`: a token surviving
    standalone is only excused when every one of its occurrences is
    accounted for by an intact occurrence of the whole original line,
    itself within the proven expected residual.
    """
    expected = plan.expected_residual_counts or {}
    residue: list[str] = []
    for group_id, texts in plan.group_value_texts.items():
        ...
        for original_line in texts:
            line_key = " ".join(original_line.split()).strip(".,;: ").lower()
            allowance = expected.get(line_key, 0)
            full_line_count = _count_value_occurrences(out_norm, original_line) if allowance else 0
            tokens = [tok.strip(".,;:()") for tok in re.split(r"[\s,]+", original_line.strip())]
            tokens = [tok for tok in tokens if tok]
            for token in tokens:
                if len(token) < 3:
                    continue
                if token.lower() in replacement_text:
                    continue
                count = _count_value_occurrences(out_norm, token)
                if count > min(allowance, full_line_count):
