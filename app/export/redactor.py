    ...
    with PDF_LOCK:
        ...
        try:
            for page_no in sorted({t.page_no for t in plan.targets}):
                ...
                # 2a. Blackout targets get a solid bar. Used where a pseudonym
                # cannot stand in - a signature, a logo, a photograph.
                for t in targets:
                    if t.blackout:
                        page.draw_rect(
                            pymupdf.Rect(t.rect), color=(0, 0, 0), fill=(0, 0, 0),
                            width=0.5, overlay=True,
                        )
                        report.inserted += 1

                # 2b. Insert the red replacement text into the cleared area.
                for t in targets:
                    if t.blackout:
                        continue
                    status = _insert_replacement(page, t, targets)
                    if status == "shrunk":
...
def _headroom_above(target, box, page_targets: list) -> float:
    """How far the box could grow upward before reaching another target's
    row on the same page. A generous default (the full symmetric pad) is
    used when nothing else is nearby, which is the overwhelmingly common case."""
    best = 999.0
    for other in page_targets:
        if other is target:
            continue
        oy1 = other.rect[3]
        if oy1 <= box.y0 and _x_overlaps(box, other.rect):
            best = min(best, box.y0 - oy1)
    return max(best, 0.0)


def _headroom_below(target, box, page_targets: list) -> float:
    best = 999.0
    for other in page_targets:
        if other is target:
            continue
        oy0 = other.rect[1]
        if oy0 >= box.y1 and _x_overlaps(box, other.rect):
            best = min(best, oy0 - box.y1)
    return max(best, 0.0)


def _x_overlaps(box, rect) -> bool:
    return not (box.x1 < rect[0] or rect[2] < box.x0)


def _insert_replacement(page: pymupdf.Page, target: Target, page_targets: list) -> str:
    """Draw red replacement text, shrinking to fit rather than clipping."""
    x0, y0, x1, y1 = target.rect
    size = max(target.font_size, MIN_FONT_SIZE)
    fontname = _safe_font(target.font_name, getattr(target, "font_flags", 0))
    ...
    for attempt_box, forced in attempts:
        ...
        if current is None:
            continue
        # Pad proportional to the TARGET's OWN height, not the font size. The
        # previous 0.35x-font-size expansion could exceed the gap to the row
        # above on tightly leaded documents, pushing replacement text into the
        # label two rows up. Sized against the box's own height instead, the
        # pad can never reach past half of the target's own row, whatever the
        # font or line spacing.
        # Only pad DOWNWARD, never up. Upward padding is what pushed replaced
        # text into the row above; ascenders on capital letters need no extra
        # clearance, and giving descenders (g, j, p, q, y) a little room below
        # is enough on its own.
        own_height = max(target.rect[3] - target.rect[1], current)
        pad = min(0.35 * current, 0.35 * own_height)
...
def _safe_font(font_name: str, flags: int = 0) -> str:
    """Map an embedded font name onto a base-14 font we can always draw.

    Bold/italic is read from PyMuPDF's FONT FLAGS bitmask first (bit 4 = bold,
    bit 1 = italic) - reliable regardless of what the font is actually named,
    which a substring check on an obfuscated embedded name (e.g.
    "Arial-CustomID7") cannot see at all. The name-substring check still runs
    as a fallback for when flags were not captured (0).
    """
    name = (font_name or "").lower()
    bold = bool(flags & (1 << 4)) or "bold" in name or "black" in name or "heavy" in name
    italic = bool(flags & (1 << 1)) or "italic" in name or "oblique" in name
