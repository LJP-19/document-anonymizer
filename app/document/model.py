...
#: One-for-one character substitutions applied at extraction. Only mappings that
#: preserve length are allowed: a ligature expanding to two characters would
#: shift every offset after it and misplace redaction rectangles.
LOOKALIKES = {
    "\u00a0": " ",  # non-breaking space
    "\u2007": " ",  # figure space
    "\u2009": " ",  # thin space
...
def normalize_char(text: str) -> str:
    """Replace lookalike characters, preserving length exactly."""
    return LOOKALIKES.get(text, text) if len(text) == 1 else text
...
class Span:
    ...
    #: PyMuPDF's font flag bitmask (bit 0 = superscript, 1 = italic, 4 = bold,
    #: 5 = serif, 6 = monospace). Preserved so a replacement keeps the
    #: original's weight and slant - color stays red regardless; that is a
    #: deliberate, verified security signal, never something this changes.
    flags: int = 0
    #: The exact baseline point PyMuPDF drew the original glyphs from.
    #: Preserved for precise vertical placement rather than deriving it from
    #: the bounding box, which loses sub-pixel baseline position.
...
class Line:
    ...
    @property
    def flags(self) -> int:
        return self.spans[0].flags if self.spans else 0

    @property
    def origin(self) -> Optional[tuple[float, float]]:
        return self.spans[0].origin if self.spans else None
