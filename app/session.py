...
class AnonymizationSession:
    ...
    def analyse(self, use_ner: bool = True, use_llm: Optional[bool] = None) -> DetectionResult:
        ...
        if use_llm is None:
            ...
            # OFF by default. The model proposed labels, headings and figures
            # as often as it found real misses, and every acceptance was one
            # more chance to damage a document. The deterministic re-scan of the
            # output covers the useful part of what it did.
            use_llm = os.environ.get("DOCANON_LLM", "0") in ("1", "true", "yes")
