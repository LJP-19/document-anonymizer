...
class Candidate:
    ...
    review_reason: str = ""
    #: True = cover permanently with a black bar instead of substituting text.
    #: Used for signatures, logos and anything a pseudonym cannot represent.
    blackout: bool = False
    #: Set on both halves of a joint name field, so the couple keeps a shared
    #: surname in the output as they had one in the original.
    household: str = ""
    #: Confirmed / probable / unresolved, from app.detection.adjudication.
    #: Defaults to "" (unset) rather than a real Adjudication value here, to
    #: avoid a circular import (adjudication.py imports Candidate). Plain
    #: string, not the enum - engine.py sets it to Adjudication.value after
    #: the adjudication pass runs; anything that never runs that pass (a
    #: hand-built Candidate in a test, an older code path) simply leaves it
    #: unset, which is treated as CONFIRMED by every reader for backward
    #: compatibility.
    adjudication: str = ""
...
class DetectionResult:
    ...
    #: Coverage/adjudication stats (spec sections 18-19). Populated by
    #: engine.py's final pass; a DetectionResult built by hand (older tests)
    #: simply leaves these at their defaults rather than breaking.
