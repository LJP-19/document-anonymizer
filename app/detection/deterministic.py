...
def load_rules(path: str = str(RULES_PATH)) -> RuleSet:
    ...
    rules = tuple(
        Rule(
            ...
            # Case-insensitive BY DEFAULT. Every rule compiled case-SENSITIVE
            # before this, while labels (below) already compiled with re.I.
            # Any pattern whose match includes letters - street suffixes, PO
            # Box, unit markers - silently failed on ALL-CAPS text, which is
            # exactly how a great deal of real government correspondence
            # renders address blocks. "4049 MICHAEL CMN" matched nothing;
            # typed PERSON instead and pseudonymized as a name.
            #
            # A rule can opt OUT with `case_sensitive: true` in the YAML. The
            # city/state rule needs this: half the two-letter state codes are
            # also common English words (OR, IN, HI, ME, OK...), and
            # case-insensitive matching turned "you, or your spouse" into a
            # detected address. Real state codes on a form are written in
            # actual capitals; the collision only exists in lowercase.
            regex=re.compile(r["pattern"], re.I if not r.get("case_sensitive") else 0),
