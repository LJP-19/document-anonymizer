...
def detect_form_text(
    ...
    # A line bound to a PII-typed field's value is NEVER chrome, however many
    # pages it repeats on. A client's name filled into the same box on every
    # voucher page of a multi-page form is exactly as "repeated" as a running
    # header by this measure, and was being vetoed outright - the client's own
    # name disappeared from EVERY page, not just the repeated ones. This is
    # the single most important exclusion in this module.
    pii_value_lines: set[tuple[int, int, int]] = set()
    for label in labels or []:
        if label.non_pii or not label.expected_types:
            continue
        pii_value_lines.add(label.line.key())
    for group in groups or []:
        if group.label.non_pii or not group.expected_types:
            continue
        for line in group.value_lines:
            pii_value_lines.add(line.key())

    # General case, not just group-bound values: ANY line a detector has
    # already put a candidate on is never eligible to be reclassified as
    # chrome for repeating across pages. A real address or SSN block printed
    # identically on every page of a multi-page return is legitimate repeated
    # PII, not a running header - and it usually has no binding label at all,
    # so the group-only exemption above missed exactly this shape.
    for candidate in candidates or []:
        pii_value_lines.add(candidate.line.key())

    # Repeated running text: the same words in the same place on several pages.
    seen: dict[tuple[str, int], list[Line]] = {}
    for page in doc.pages:
        for line in page.lines:
            if line.key() in pii_value_lines:
                continue
