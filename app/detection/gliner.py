...
LABELS: dict[str, Optional[PiiType]] = {
    ...
    # A client's own business is identity; a bank or agency named in passing is
    # not. The label wording is deliberately narrow for that reason.
    "client business name": PiiType.ORG_PRIVATE,
    "employer name": PiiType.ORG_PRIVATE,
...
def _page_words(page) -> list[tuple[str, object, int, int]]:
    """Every word on the page in reading order, with its line and offsets."""
    words: list[tuple[str, object, int, int]] = []
    for line in page.lines_in_reading_order():
        for word, start, end in _words_with_offsets(line.text):
            words.append((word, line, start, end))
    return words
...
def detect_gliner(
    ...
    Runs over a sliding window of the PAGE in reading order, not over one layout
    block at a time. A block on a form is frequently a single line, which left
    the model classifying an isolated fragment with nothing around it - the most
    likely reason names in form fields were being missed or mislabelled.
    ...
    for page in doc.pages:
        ...
        step = max(1, MAX_WORDS_PER_CHUNK - CHUNK_OVERLAP)
        seen: set[tuple] = set()
        for chunk_start in range(0, len(words), step):
            ...
            if not chunk:
                break
            try:
                spans = detector.predict([w for w, _l, _s, _e in chunk])
            except Exception as exc:  # noqa: BLE001 - never break analysis
                log.warning("GLiNER inference failed on a page: %s", type(exc).__name__)
                warnings.append(f"GLiNER inference failed on page {page.number + 1}")
                break

            for span in spans:
                first, last = chunk[span.start_word], chunk[span.end_word]
                line = first[1]
                # A span that runs across two visual lines cannot be given one
                # rectangle; keep the part on the line it started.
                if last[1] is not line:
                    end_offset = len(line.text.rstrip())
                else:
                    end_offset = last[3]
                ...
                key = (page.number, line.key(), start_offset, end_offset, span.label)
                if key in seen:
                    continue
                seen.add(key)

                rect = line.rect_for(start_offset, end_offset)
                if rect is None:
                    continue
                ...
                candidates.append(
                    Candidate(
                        ...
                        text=line.text[start_offset:end_offset],
                        page_no=page.number,
                        rect=rect,
                        line=line,
                        start=start_offset,
                        ...
                        source=Source.NER,
                        evidence=[Evidence(Source.NER, f"GLiNER: {span.label}", span.score)],
                        needs_review=span.score < 0.62,
                        review_reason="" if span.score >= 0.62 else "low model confidence",
                    )
                )
