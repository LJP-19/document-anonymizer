...
#: A candidate whose rect overlaps a label's rect by at least this fraction of
#: its OWN area is dropped outright, whatever line either belongs to.
SPATIAL_LABEL_OVERLAP_THRESHOLD = 0.5


def veto_by_label_geometry(
    candidates: list[Candidate], labels: list[LabelRegion]
) -> tuple[list[Candidate], int]:
    """A true bounding-box safety net, independent of which Line either
    side belongs to (spec sections 13 and 85).

    `strip_label_overlaps` only clips overlap within the SAME Line object, by
    character offset. That misses two real cases: text extraction sometimes
    splits one visual line into two Line objects with no character-offset
    relationship at all, and a proposal layer (Presidio, GLiNER) can return a
    span whose LINE differs from the label's own Line even though its
    RECTANGLE genuinely overlaps the label on the page. This checks geometry
    directly - a candidate is dropped if its rect shares at least half its own
    area with any label's rect, full stop.
    """
    dropped = 0
    kept: list[Candidate] = []
    for candidate in candidates:
        area = _area(candidate.rect)
        hit = False
        if area > 0:
            for label in labels:
                if _overlap_area(candidate.rect, label.rect) / area >= SPATIAL_LABEL_OVERLAP_THRESHOLD:
                    hit = True
                    break
        if hit:
            dropped += 1
