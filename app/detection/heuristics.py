...
def _scan_line(line: Line, on_line: list[Candidate]) -> list[Candidate]:
    ...
    if not is_name and len(segment.split()) == 1 and looks_like_a_lone_surname(segment):
        # The whole line is one capitalised word: a name cell, not prose.
        is_name, confidence = True, 0.5
    if not is_name:
        return results
