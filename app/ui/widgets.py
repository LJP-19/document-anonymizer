...
class TaskRunner(QObject):
    ...
    def stop(self) -> None:
        """Quit and join a running thread, then drop the reference.

        Two failure modes had to be closed at once. Keeping the reference after
        stopping let Qt destroy a live QThread at interpreter exit (abort, code
        134). Touching the thread unconditionally dereferenced an object Qt had
        already started destroying (segfault, code 139). So: drop the reference
        first, then act only on a thread that is still valid AND still running.
        """
        ...
        try:
            ...
            pass  # the C++ object went away underneath us; nothing to join
