...
class TaskRunner(QObject):
    ...
    def stop(self) -> None:
        """Quit and join unconditionally, then drop the reference.

        Checking isRunning() first is not enough: a thread that has finished its
        event loop but not yet been joined is still a live QThread, and Qt
        aborts the process if one is destroyed in that state.
        """
