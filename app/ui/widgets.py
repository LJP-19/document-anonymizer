...
class DetectionCard(QFrame):
    ...
    def __init__(self, group: OccurrenceGroup, state: DecisionState, flagged: bool,
        ...
        self.remove_button.setToolTip("Remove this detection - leave the text as it is")
        self.remove_button.clicked.connect(lambda: self.removed.emit(self.group))
...
class PageView(QLabel):
    ...
    undo_requested = Signal(int, tuple)

    #: Side of the little dismiss badge drawn on each applied change.
    BADGE = 14
    ...
    def set_badges(self, enabled: bool) -> None:
        """Show a dismiss badge on every applied change."""
        self.show_badges = enabled
        self.repaint_overlays()
    ...
    def _badge_rect(self, rect) -> tuple[int, int, int, int]:
        x0, y0, x1, _y1 = (v * self._zoom for v in rect)
        size = self.BADGE
        return (int(x1) - size // 2, int(y0) - size, size, size)

    #: Extra pixels of forgiveness around the visible badge, each direction.
    #: A 14x14 badge with no slack is a hard target to actually hit, which is
    #: enough on its own to make the "x" appear broken even when the click
    #: handler itself is correct.
    HIT_SLOP = 6

    def _badge_hit(self, point) -> Optional[tuple]:
        if not self.pixmap():
            return None
        pixmap = self.pixmap()
        offset_x = max(0, (self.width() - pixmap.width()) // 2)
        offset_y = max(0, (self.height() - pixmap.height()) // 2)
        x, y = point.x() - offset_x, point.y() - offset_y
        for rect, _kind, _focused in self._overlays:
            bx, by, bw, bh = self._badge_rect(rect)
            bx, by = bx - self.HIT_SLOP, by - self.HIT_SLOP
            bw, bh = bw + 2 * self.HIT_SLOP, bh + 2 * self.HIT_SLOP
            if bx <= x <= bx + bw and by <= y <= by + bh:
                return rect
        return None

    def mousePressEvent(self, event):  # noqa: N802 - Qt naming
        if self.show_badges and event.button() == Qt.LeftButton:
            hit = self._badge_hit(event.position().toPoint())
            if hit is not None:
                self.undo_requested.emit(self.page_no, hit)
                return
        ...
        if self._overlays:
            ...
            if self.show_badges:
                for rect, _kind, _focused in self._overlays:
                    ...
                    painter.drawLine(bx + 4, by + 4, bx + bw - 4, by + bh - 4)
                    painter.drawLine(bx + bw - 4, by + 4, bx + 4, by + bh - 4)
            painter.end()
...
class DocumentView(QScrollArea):
    ...
    def build(self, pages: list[int]) -> None:
        ...
        for page_no in pages:
            ...
            view.set_badges(self.badges)
            view.region_drawn.connect(self.region_drawn)
            view.undo_requested.connect(self.undo_requested)
