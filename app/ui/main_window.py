...
class DocumentTab(QWidget):
    ...
    def _sidebar(self) -> QWidget:
        ...
        for key, label in (
            ("flagged", "To review"), ("second", "2nd pass"),
    ...
    def _is_second_pass(self, group: OccurrenceGroup) -> bool:
        """Found by the check that re-reads the anonymized output."""
        from ..detection.types import Source

        return any(c.source is Source.AUDIT for c in group.candidates)
    ...
    def visible_groups(self) -> list[OccurrenceGroup]:
        groups = self.groups()
        if self.filter_mode == "second":
            groups = [g for g in groups if self._is_second_pass(g)]
    ...
    def rebuild_cards(self) -> None:
        ...
        if not groups:
            message = {
                ...
                "second": "The second pass found nothing still readable in the output.",
    ...
    def _update_chips(self) -> None:
        ...
        second = len([g for g in groups if self._is_second_pass(g)])
        self.chips["second"].setText(f"2nd pass \u00b7 {second}")
