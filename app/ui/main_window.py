...
from . import theme
from .widgets import KEEP, REDACT, DetectionCard, DocumentView, TaskRunner
    ...
    def rebuild_cards(self) -> None:
        ...
        if not groups:
            ...
            for group in groups:
                ...
                card.reviewed_changed.connect(self.mark_reviewed)
...
class MainWindow(QMainWindow):
    ...
    def process(self) -> None:
        ...
        if not approved:
            return
        unreviewed = sum(i.flagged for i in approved)
        if unreviewed:
            answer = QMessageBox.question(
                self,
                "Process without reviewing?",
                f"{unreviewed} flagged item(s) across the approved documents have not been "
                "reviewed. They will be pseudonymized as detected.\n\nContinue?",
            )
    ...
    def _report(self, progress: BatchProgress) -> None:
        ...
        self.queue_label.setText(progress.text + suffix)
        if progress.total:
