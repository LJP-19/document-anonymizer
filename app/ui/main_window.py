...
def slide_in(widget: QWidget, distance: int = 14, duration: int = 190) -> None:
    """Nudge a widget into place. Used when a list is rebuilt."""
    if not shiboken6.isValid(widget):
        return
    start = widget.pos()
    animation = QPropertyAnimation(widget, b"pos", widget)
    animation.setDuration(duration)
    animation.setStartValue(start + QPoint(0, distance))
    animation.setEndValue(start)
    animation.setEasingCurve(QEasingCurve.OutCubic)
    animation.start(QPropertyAnimation.DeleteWhenStopped)
    widget._slide = animation


def flash(widget: QWidget, colour: str, duration: int = 420) -> None:
    """Briefly tint a widget so a decision is visibly acknowledged."""
    if not shiboken6.isValid(widget):
        return
    ...
    QTimer.singleShot(
        duration,
        lambda: widget.setStyleSheet(original) if shiboken6.isValid(widget) else None,
    )
...
class DocumentTab(QWidget):
    ...
    def __init__(self, item: BatchItem):
        ...
        self.final_review = False
        splitter.addWidget(self.sidebar)
        self.final_sidebar = self._final_review_sidebar()
        self.final_sidebar.setVisible(False)
        splitter.addWidget(self.final_sidebar)
    ...
    def _sidebar(self) -> QWidget:
        ...
        for index, (key, label) in enumerate((
            ("flagged", "To review"), ("unlabelled", "Unlabelled"),
            ("reviewed", "Done"), ("all", "All"), ("kept", "Kept")
        )):
            ...
            # Two per row, so a narrow panel wraps rather than clipping.
        ...
        keep_all.setToolTip("Leave every detected value in the document unchanged")
    ...
    def _final_review_sidebar(self) -> QWidget:
        """Every detected item, listed for a last check - not the full working
        panel. Each row still supports Keep / Redact / Edit, same as review."""
        side = QWidget()
        side.setObjectName("Sidebar")
        side.setMinimumWidth(300)
        side.setMaximumWidth(560)
        ...
        heading = QLabel("Final review")
        heading.setObjectName("DocTitle")
        column.addWidget(heading)

        self.final_summary = QLabel("")
        self.final_summary.setObjectName("CardMeta")
        self.final_summary.setWordWrap(True)
        column.addWidget(self.final_summary)

        self.final_list_area = QScrollArea()
        self.final_list_area.setWidgetResizable(True)
        self.final_list_host = QWidget()
        self.final_list_layout = QVBoxLayout(self.final_list_host)
        self.final_list_layout.setContentsMargins(0, 0, 6, 0)
        self.final_list_layout.setSpacing(8)
        self.final_list_layout.addStretch(1)
        self.final_list_area.setWidget(self.final_list_host)
        column.addWidget(self.final_list_area, 1)

        self.process_here_button = QPushButton("Process \u2192")
        self.process_here_button.setEnabled(False)
        self.process_here_button.setObjectName("Primary")
        column.addWidget(self.process_here_button)
        return side

    def rebuild_final_list(self) -> None:
        """Every group, in one list, still editable - the last chance to change
        anything before Process is pressed.

        takeAt() detaches a widget from the layout IMMEDIATELY, before
        deleteLater() actually frees it on the next event loop turn. A rapid
        second rebuild (a decision made right after entering final review, in
        the same call stack) that instead trusted layout.count() to already
        reflect an old removeWidget()/deleteLater() call touched a widget mid
        deletion and crashed the process.
        """
        while self.final_list_layout.count() > 1:
            item = self.final_list_layout.takeAt(0)
            widget = item.widget() if item else None
            if widget is not None:
                widget.deleteLater()

        groups = self.groups()
        redacted = len([g for g in groups if self._state_of(g) is not KEEP])
        kept = len([g for g in groups if self._state_of(g) is KEEP])
        self.final_summary.setText(
            f"{len(groups)} distinct value(s) \u00b7 {redacted} will be pseudonymized "
            f"\u00b7 {kept} kept as-is. Anything here can still be changed."
        )
        for group in groups:
            card = DetectionCard(group, self._state_of(group), group.needs_review, True)
            ...
            self.final_list_layout.insertWidget(self.final_list_layout.count() - 1, card)

        # "Process (#)" only once at least one item has actually been decided
        # here - not a static label that implies readiness before anything
        # has been approved in this pass.
        decided = len([g for g in groups if self._is_reviewed(g)])
        self.process_here_button.setEnabled(decided > 0)
        self.process_here_button.setText(
            f"Process ({decided}) \u2192" if decided else "Process \u2192"
        )
    ...
    def _viewer(self) -> QWidget:
        ...
        manual = QWidget()
        manual.setObjectName("Toolbar")
        manual.setFixedHeight(46)
        strip = QHBoxLayout(manual)
        strip.setContentsMargins(14, 8, 14, 8)
        strip.setSpacing(6)

        self.search_doc = QLineEdit()
        self.search_doc.setPlaceholderText("Find text in the document\u2026")
        self.search_doc.returnPressed.connect(self.search_and_pseudonymize)
        strip.addWidget(self.search_doc, 1)

        find_pseudo = QPushButton("Pseudonymize")
        find_pseudo.clicked.connect(self.search_and_pseudonymize)
        find_black = QPushButton("Black out")
        find_black.clicked.connect(self.search_and_blackout)
        strip.addWidget(find_pseudo)
        strip.addWidget(find_black)

        self.final_button = QPushButton("Go to final review \u203a")
        self.final_button.setObjectName("Primary")
        self.final_button.setCheckable(True)
        self.final_button.toggled.connect(self.set_final_review)
        strip.addWidget(self.final_button)

        self.box_button = QPushButton("Draw blackout box")
        self.box_button.setObjectName("Chip")
        self.box_button.setCheckable(True)
        self.box_button.toggled.connect(self.set_box_mode)
        strip.addWidget(self.box_button)
        ...
        self.left_canvas.region_drawn.connect(self.on_region_drawn)
        self.right_canvas.undo_requested.connect(self.on_undo_requested)
    ...
    def _is_unlabelled(self, group: OccurrenceGroup) -> bool:
        """Found, but nothing could say what it is - so nothing replaces it."""
        from ..detection.types import PiiType

        return group.pii_type is PiiType.UNCLASSIFIED_GROUP_VALUE
    ...
    def visible_groups(self) -> list[OccurrenceGroup]:
        groups = self.groups()
        if self.filter_mode == "unlabelled":
            groups = [g for g in groups if self._is_unlabelled(g)]
        elif self.filter_mode == "flagged":
            groups = [g for g in groups if not self._is_reviewed(g) and not self._is_unlabelled(g)]
    ...
    def rebuild_cards(self) -> None:
        ...
        if not groups:
            message = {
                ...
                "unlabelled": (
                    "Nothing unidentified. Every detected value has a type, so every "
                    "one has a matching replacement."
                ),
            ...
            slide_in(self.list_host, 10, 170)
        self._update_chips()
        self.changed.emit()
    ...
    def _update_chips(self) -> None:
        groups = self.groups()
        flagged = len(
            [g for g in groups if not self._is_reviewed(g) and not self._is_unlabelled(g)]
        )
        reviewed = len([g for g in groups if self._is_reviewed(g)])
        kept = len([g for g in groups if self._state_of(g) is KEEP])
        unlabelled = len([g for g in groups if self._is_unlabelled(g)])
        self.chips["unlabelled"].setText(f"Unlabelled \u00b7 {unlabelled}")
    ...
    def select_group(self, group: OccurrenceGroup, scroll: bool = True) -> None:
        ...
        self.rebuild_views()
        if scroll:
            # Same reason as jump_to_page: the views were just rebuilt.
            QTimer.singleShot(0, lambda: self._scroll_both(first.page_no, first.rect))
            QTimer.singleShot(180, lambda: self._scroll_both(first.page_no, first.rect))

    def jump_to_page(self, group: OccurrenceGroup, page_no: int) -> None:
        """Show the value's pages, then scroll to the one that was clicked.

        select_group rebuilds both scrollers, and a widget that has not been
        laid out yet reports position 0 - so the scroll has to wait a turn.
        """
        self.select_group(group, scroll=False)
        QTimer.singleShot(0, lambda: self._scroll_both(page_no))
        QTimer.singleShot(180, lambda: self._scroll_both(page_no))

    def _scroll_both(self, page_no: int, rect=None) -> None:
        if not shiboken6.isValid(self):
            return
        self.left_canvas.scroll_to_page(page_no, rect, self.zoom)
        self.right_canvas.scroll_to_page(page_no, rect, self.zoom)
    ...
    def _flash_card(self, group: OccurrenceGroup, colour: str) -> None:
        for card in self.cards:
            if (card.group.pii_type, card.group.normalized) == (group.pii_type, group.normalized):
                flash(card, colour)
                break
    ...
    def decide_group(self, group: OccurrenceGroup, state: DecisionState) -> None:
        if state is DecisionState.EDITED:
            ...
            if dialog.exec() != QDialog.DialogCode.Accepted:
                return
            from .dialogs import coerce_type

            new_type, replacement, apply_all = dialog.result_values()
            new_type = coerce_type(new_type)
            edited_value = dialog.edited_value()

            # The detected span itself can be wrong. Correcting it re-adds the
            # value as the user typed it and drops the bad span.
            if edited_value and edited_value.lower() != group.normalized.lower():
                self.session.decisions.set_state(group.candidates, KEEP)
                ...
                if not added:
                    QMessageBox.information(
                        self, "Not found",
                        f"\u201c{edited_value}\u201d does not appear in this document.",
                    )
                    self.session.decisions.set_state(group.candidates, REDACT)
                self._after_decision()
                return

            if apply_all:
                # A confirmed value is swept across the WHOLE document, not
                # just the occurrences already caught. Matching only against
                # existing candidates here is exactly how "Mark Lang" typed
                # correctly on page 1 stayed missed on page 3 - that page's
                # occurrence was never a candidate at all, so nothing existing
                # to match against. add_manual_text's cascade search adopts
                # every match it already knows about AND finds new ones.
                targets = self.session.add_manual_text(
                    group.normalized,
                    replacement=replacement or None,
                    pii_type=new_type,
                    cascade=True,
        ...
        # Anything co-occurring with this value - the same stacked field, the
        # same line - is already known to belong together, so a decision on
        # one carries a "reviewed" mark to the rest. This never changes their
        # OWN decision, only that they no longer need a separate look.
        if state is not DecisionState.EDITED:
            from ..detection.entity_graph import build_entity_graph

            graph = build_entity_graph(self.session.candidates, self.session.detection.groups)
            for member in group.candidates:
                for linked in graph.linked(member):
                    self.session.decisions.mark_reviewed([linked], True)

        self._flash_card(group, theme.OK if state is REDACT else theme.TEXT_FAINT)
        self._after_decision()

    def remove_group(self, group: OccurrenceGroup) -> None:
        """Drop a detection: the text stays exactly as it is.

        Different from Keep only in intent - Keep is a decision about a real
        value, this says the detector was wrong to flag it at all.
        """
        self._guard("Remove detection", lambda: self._remove_group(group))

    def _remove_group(self, group: OccurrenceGroup) -> None:
        ids = {c.id for c in group.candidates}
        self.session.decisions.set_state(group.candidates, KEEP)
        self.session.decisions.mark_reviewed(group.candidates, True)
        self.session.detection.candidates = [
            c for c in self.session.detection.candidates if c.id not in ids
        ]
        self._after_decision()
    ...
    def set_final_review(self, enabled: bool) -> None:
        """Last look before writing: manual tools, and every change dismissable.

        The detection list steps aside. What is left is the document as it will
        be written, with an x on every change, plus search, draw and add.
        """
        self.final_review = enabled
        self.sidebar.setVisible(not enabled)
        self.final_sidebar.setVisible(enabled)
        if enabled:
            self.rebuild_final_list()
        self.right_canvas.set_badges(enabled)
        self.set_view("redacted" if enabled else "split")
        self.final_button.setText(
            "\u2039 Back to review" if enabled else "Go to final review \u203a"
        )
        self.changed.emit()
        if enabled:
            fade_in(self.right_canvas, 220)

    def on_undo_requested(self, page_no: int, rect: tuple) -> None:
        """The x on a change in the preview: put that value back.

        Matched with a tolerance, not exact float equality - the rect a click
        reports has been through a widget-to-PDF coordinate conversion and a
        zoom multiply/divide round trip, and exact equality against the
        original candidate rect is not guaranteed to survive that.
        """
        def close(a: tuple, b: tuple, eps: float = 0.75) -> bool:
            return all(abs(x - y) <= eps for x, y in zip(a, b))

        matches = [
            c for c in self.session.candidates
            if c.page_no == page_no and close(c.rect, rect)
        ]
        if not matches:
            return
        self.session.decisions.set_state(matches, KEEP)
        self.session.decisions.mark_reviewed(matches, True)
        self._after_decision()

    def set_box_mode(self, enabled: bool) -> None:
        self.left_canvas.set_box_mode(enabled)
        self.box_button.setText(
            "Drawing \u2014 click again to stop" if enabled else "Draw blackout box"
        )

    def on_region_drawn(self, page_no: int, rect: tuple) -> None:
        """A drawn area is blacked out: a pseudonym cannot stand in for a mark."""
        added = self.session.add_manual_region(page_no, rect)
        if added is None:
            return
        self._after_decision()

    def _search_and_add(self, blackout: bool) -> None:
        needle = self.search_doc.text().strip()
        if not needle or not self.session:
            return
        hits = self.session.find_text(needle)
        if not hits:
            QMessageBox.information(
                self, "Not found",
                f"\u201c{needle}\u201d does not appear in this document's text.\n\n"
                "If the page is a scan, its text cannot be searched.",
            )
            return
        added = self.session.add_manual_text(needle, blackout=blackout)
        if not added:
            QMessageBox.information(
                self, "Already handled", f"\u201c{needle}\u201d is already in the list."
            )
            return
        self._after_decision()
        action = "blacked out" if blackout else "pseudonymized"
        QMessageBox.information(
            self, "Added",
            f"{len(added)} occurrence(s) of \u201c{needle}\u201d will be {action}.",
        )

    def search_and_pseudonymize(self) -> None:
        self._guard('Search and pseudonymize', lambda: self._search_and_pseudonymize())

    def _search_and_pseudonymize(self) -> None:
        self._search_and_add(blackout=False)

    def search_and_blackout(self) -> None:
        self._guard('Search and black out', lambda: self._search_and_blackout())

    def _search_and_blackout(self) -> None:
        self._search_and_add(blackout=True)

    def add_missed(self) -> None:
        self._guard('Add missed item', lambda: self._add_missed())

    def _add_missed(self) -> None:
        ...
        if not added:
            ...
            return
        # Show it immediately. Marking it reviewed sent it straight to Done,
        # where the user could not see that anything had happened.
        self.session.decisions.mark_reviewed(added, False)
        self.set_filter("all")
        self.rebuild_views()
        self._after_decision()
        for card in self.cards:
            if card.group.normalized.lower() == added[0].normalized.lower():
                self.select_group(card.group)
                break
        QMessageBox.information(
            self, "Added",
    ...
    def keep_all(self) -> None:
        self._guard("Keep everything", lambda: self._keep_all())
    ...
    def _keep_all(self) -> None:
        ...
        answer = QMessageBox.question(
            self,
            "Keep everything?",
            f"All {len(groups)} detected value(s) will be left in the document "
            "unchanged, and moved to Done.\n\nNothing is written until you press Process.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No,
        )
        if answer != QMessageBox.StandardButton.Yes:
            return
        self.session.decisions.set_state(self.session.candidates, KEEP)
        self.session.decisions.mark_reviewed(self.session.candidates, True)
        self._after_decision()

    def redact_all(self) -> None:
        self._guard('Redact everything', lambda: self._redact_all())

    def _redact_all(self) -> None:
        groups = self.groups()
        pending = [g for g in groups if not self._is_reviewed(g)]
        answer = QMessageBox.question(
            self,
            "Redact everything?",
            f"{len(pending)} item(s) still awaiting review will be pseudonymized "
            f"and moved to Done.\n\nNothing is written until you press Process.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No,
        )
        ...
        # Deciding is reviewing: everything moves to Done, as it does for a
        # single item.
        self.session.decisions.mark_reviewed(self.session.candidates, True)
        self._after_decision()
    ...
    def _guard(self, what: str, action) -> bool:
        """Run a review action, reporting failure instead of doing nothing.

        An exception inside a Qt slot is discarded: the button appears to work
        and nothing happens. Every user-triggered action goes through here.
        """
        try:
            action()
            return True
    ...
    def _after_decision(self) -> None:
        """Refresh everything a decision can change.

        Only re-queueing the preview left the rendered pages stale whenever the
        set of visible pages changed - which is why edits and additions did not
        appear to do anything.
        """
        self.rebuild_cards()
        self.draw_overlays()
        if getattr(self, "final_review", False):
            self._draw_applied_badges()
            self.rebuild_final_list()
        self._preview_timer.stop()
        self._refresh_preview()

    def _draw_applied_badges(self) -> None:
        """Mark every change that will actually be written, so it can be undone."""
        if not self.session or not self.session.detection:
            return
        applied = {t.candidate_id for t in self.session.plan().targets}
        by_page: dict[int, list] = {p: [] for p in self.visible_pages()}
        for candidate in self.session.candidates:
            if candidate.id in applied and candidate.page_no in by_page:
                by_page[candidate.page_no].append((candidate.rect, "redact", False))
        for page_no, overlays in by_page.items():
            self.right_canvas.set_overlays(page_no, overlays)
    ...
    def rebuild_views(self) -> None:
        ...
        if show_redacted:
            ...
            if getattr(self, "final_review", False):
                QTimer.singleShot(260, self._draw_applied_badges)
...
class ResultsView(QWidget):
    ...
    def show_results(self, items: list[BatchItem], output_folder: Optional[str]) -> None:
        ...
        where = output_folder or (
            str(Path(self.items[0].output_path).parent) if self.items and self.items[0].output_path
            else "their source folders"
        )
        written = "\n".join(
            f"  \u2022 {Path(i.output_path).name}" for i in self.items if i.output_path
        )
        self.summary.setText(
            f"{len(self.items)} document(s) written to:\n{where}\n{written}\n\n"
            f"{total} distinct value(s) replaced \u00b7 {verified} passed export verification.\n"
...
class MainWindow(QMainWindow):
    ...
    def _build(self) -> None:
        ...
        # Wired per-tab once the tab exists; see DocumentTab construction.
    ...
    def _folders(self) -> QWidget:
        ...
        self.output_edit.setPlaceholderText("Choose where anonymized files are written")
        default = Path.home() / "Documents" / "Anonymized"
        self.output_edit.setText(str(default))
        self.batch.output_folder = str(default)
        ...
        self.conservative_box = QCheckBox("Careful mode")
        self.conservative_box.setToolTip(
            "Only change values that were positively identified.\n"
            "Finds less, but will not alter figures or form text.\n"
            "Use Add missed item for anything it leaves behind."
        )
        import os

        self.conservative_box.setChecked(
            os.environ.get("DOCANON_CONSERVATIVE", "0") in ("1", "true", "yes")
        )
        self.conservative_box.toggled.connect(self._set_conservative)
        row.addWidget(self.conservative_box)
        return bar

    def _set_conservative(self, enabled: bool) -> None:
        import os

        os.environ["DOCANON_CONSERVATIVE"] = "1" if enabled else "0"
        if self.batch.items:
            QMessageBox.information(
                self, "Applies to the next analysis",
                "Careful mode changes what gets detected, so it takes effect when a "
                "document is next analysed. Re-add anything already open.",
            )
    ...
    def _queue(self) -> QWidget:
        ...
        self.process_button = QPushButton("Continue to final review")
    ...
    def add(self, items: list[BatchItem]) -> None:
        for item in items:
            ...
            # Processing the panel's own button writes every approved document,
            # same as the footer button - a batch is written together.
            tab.process_here_button.clicked.connect(
                lambda _checked=False: self._process_after_final_review(self.batch.approved)
            )
    ...
    def _refresh_queue(self) -> None:
        ...
        if failed:
            # The exact bar the user reported seeing ("1 failed") gave no
            # way to see why. One or two lines of the real error(s), right
            # on the summary itself - no click, no hunting through a log
            # file for something that was already captured.
            detail = "\n".join(
                f"{item.name}: {item.error}" for item in failed[:3] if item.error
            )
            self.queue_label.setToolTip(detail or "See each failed tab for details.")
        else:
            self.queue_label.setToolTip("")
        self.process_button.setEnabled(bool(approved))
        self.process_button.setText(
            f"Continue to final review ({len(approved)})"
            if approved else "Continue to final review"
        )
        for index in range(self.tabs.count()):
            ...
            if isinstance(tab, DocumentTab):
                ...
                # A failed item previously showed only "! name" with no way
                # to see why - item.error already captured the real
                # exception message, it just never reached the GUI. A
                # tooltip on the tab is the least invasive place to put it:
                # no new dialog, no extra click, just hover where the
                # failure marker already is.
                if tab.item.state is ItemState.FAILED and tab.item.error:
                    self.tabs.setTabToolTip(index, f"Failed: {tab.item.error}")
                else:
    ...
    def process(self) -> None:
        ...
        # Final review is mandatory. Pressing Process from the footer sends the
        # user there first, on the first approved tab that has not been through
        # it yet; the actual write only happens from that stage's own button.
        pending_review = [
            tab for tab in self._tabs_for(approved)
            if not getattr(tab, "final_review", False)
        ]
        if pending_review:
            first = pending_review[0]
            index = self.tabs.indexOf(first)
            if index >= 0:
                self.tabs.setCurrentIndex(index)
            self.stack.setCurrentWidget(self.tabs)
            first.set_final_review(True)
            return

        self._process_after_final_review(approved)

    def _tabs_for(self, items) -> list:
        wanted = {item.source_path for item in items}
        return [
            tab for tab in self.tabs_by_item.values()
            if tab.item.source_path in wanted
        ]

    def _process_after_final_review(self, approved: list) -> None:
