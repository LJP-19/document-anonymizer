...
def test_findings_are_marked_and_not_applied(tmp_path):
    ...
    for finding in result.findings:
        assert finding.source is Source.AUDIT
        assert finding.needs_review
        assert "second pass" in finding.review_reason
    ...
    for finding in result.findings:
        assert fresh.state(finding) is DecisionState.SKIPPED, (
            "a second-pass finding was applied without a decision"
        )
