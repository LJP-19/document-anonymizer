...
def test_stacked_field_all_lines_transformed(pdfs, tmp_path):
    ...
    # "LJP" alone on a line under a multi-type label cannot be typed, so it is
    # surfaced under Unlabelled rather than replaced with something arbitrary.
    # The heuristic that used to claim it also misread a great deal of a form.
    unlabelled = {
        c.normalized for c in session.candidates
        if c.pii_type is PiiType.UNCLASSIFIED_GROUP_VALUE
    }
    assert "LJP" in unlabelled, unlabelled
