...
# --- organizational-suffix guard on propagation -----------------------------


def test_a_known_persons_name_is_not_propagated_into_a_business_name(tmp_path):
    """The exact canonical example from the original spec: 'Name: LJP'
    should not transform 'LJP Manufacturing Inc.' into the PERSON entity.
    Word-boundary matching alone is not enough - 'LJP' inside 'LJP
    Manufacturing Inc.' passes a word-boundary check cleanly, since it IS a
    genuine standalone token, not a substring fragment. The failure mode is
    semantic, not lexical: propagating the person into what reads as a
    different entity's name.
    """
    from reportlab.lib.pagesizes import letter
    from reportlab.pdfgen import canvas

    # A real, unambiguous person's name - not a short all-caps acronym like
    # "LJPS", which a small NER model can initially classify as ORG_PRIVATE
    # before a later pass corrects it to PERSON. Propagation runs BEFORE
    # that correction, so a name ambiguous enough to be typed as an
    # organization at that point never seeds a subject at all - a real,
    # separate pipeline-ordering fact, not a flaw in the guard being tested
    # here. A name unambiguous enough to be typed PERSON immediately by
    # every layer isolates the one thing this test is actually about.
    path = tmp_path / "marcus.pdf"
    c = canvas.Canvas(str(path), pagesize=letter)
    c.setFont("Helvetica", 10)
    c.drawString(72, 720, "Name:")
    c.drawString(72, 706, "Marcus Feldman")
    # A real gap wider than the field-binding threshold (14pt) after the
    # value line, so the group builder stops the "Name:" field at the name
    # rather than binding the unrelated prose sentences below it into one
    # giant stacked field - the exact shape a real form has (a tight
    # label+value pair, then a clear gap before body text resumes).
    c.drawString(72, 680, "Invoice submitted by Marcus Feldman Manufacturing Inc. for services rendered.")
    c.drawString(72, 660, "Marcus Feldman confirmed receipt of the shipment on the same day.")
    c.save()
    s = analysed(str(path))
    from app.decisions.manager import DecisionState
    ...
    out_text = pymupdf.open(str(tmp_path / "out.pdf"))[0].get_text()
    assert "Marcus Feldman Manufacturing Inc." in out_text, (
        "the business name was altered - the person's name was wrongly "
        "propagated into a different entity's name"
    )
    # But the genuine second occurrence of the SAME person, standing alone,
    # must still be caught - this is not a blanket refusal to propagate.
    assert "Marcus Feldman confirmed receipt" not in out_text
    assert report.status == "EXPORT VERIFIED", report.status


def test_organizational_suffix_guard_does_not_block_a_genuine_repeat(tmp_path):
    """The guard must be scoped to org-suffix context specifically, not
    become an excuse to under-propagate a real second occurrence."""
    from app.detection.entities_pass import ORG_SUFFIX_AFTER

    assert not ORG_SUFFIX_AFTER.match(" confirmed receipt of the shipment")
    assert ORG_SUFFIX_AFTER.match(" Manufacturing Inc. for services")
    assert ORG_SUFFIX_AFTER.match(", LLC and its affiliates")


# --- verifier: expected residuals for correctly-unredacted occurrences -----


def test_verifier_allows_a_proven_expected_residual_but_not_a_real_leak(tmp_path):
    """A value can legitimately occur more times in the source than were
    ever accepted as candidates - the organizational-suffix guard above is
    exactly this case. The verifier must know the difference between that
    and an actual leak, or it reports EXPORT VERIFIED as failed for a
    document that was handled completely correctly.
    """
    from reportlab.lib.pagesizes import letter
    from reportlab.pdfgen import canvas
    ...
    s.decisions.set_state(s.candidates, DecisionState.ACCEPTED)
    plan = s.plan()
    assert plan.expected_residual_counts, "no expected residual was computed at all"

    _apply, report = s.process(str(tmp_path / "out.pdf"))
    failed = [c.name for c in report.checks if not c.passed]
    assert not failed, f"false alarm(s) on a correctly-handled document: {failed}"


def test_verifier_still_catches_a_genuine_half_redacted_name():
    """The narrow allowance must never extend to a multi-word value where
    only part of it was actually removed - that is the exact danger this
    check exists to catch, and it must stay strict."""
    from app.verification.verifier import _count_value_occurrences, _normalize

    out = _normalize("The report was filed by REDACTED Smith on behalf of the estate.")
    assert _count_value_occurrences(out, "Smith") == 1
