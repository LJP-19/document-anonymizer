...
def test_careful_mode_excludes_the_widening_passes(tmp_path, monkeypatch):
    ...
    for candidate in careful.candidates:
        ...
        assert candidate.source not in (Source.COVERAGE, Source.AUDIT, Source.GROUP)
...
def test_padding_is_symmetric_by_default_like_v0_25(tmp_path):
    """Reported: sizing/padding was better before it was made asymmetric.

    The asymmetric, own-height-scaled version under-padded ordinary documents
    to guard against a collision that only happened on one unusually tight
    synthetic fixture. Restored to symmetric padding, with a cap computed from
    the REAL neighbouring targets on the page rather than a guess.
    """
    from app.export.redactor import _headroom_above, _headroom_below
    from app.transform.plan import Target

    def target(rect):
        return Target(candidate_id=str(rect), page_no=0, rect=rect, original="x",
                       replacement="y", font_size=10.0, font_name="helv")

    lone = target((10, 40, 60, 54))
    # With nothing nearby, headroom is generous - the symmetric default holds.
    assert _headroom_above(lone, __import__("pymupdf").Rect(10, 40, 60, 54), [lone]) > 5
    assert _headroom_below(lone, __import__("pymupdf").Rect(10, 40, 60, 54), [lone]) > 5

    close_above = target((10, 20, 60, 33))
    room = _headroom_above(
        lone, __import__("pymupdf").Rect(10, 40, 60, 54), [lone, close_above]
    )
    assert abs(room - (40 - 33)) < 0.01, room
