...
def _window(tmp_path, count: int = 2) -> MainWindow:
    ...
    for item in items:
        ...
        # Mirror MainWindow.add() exactly, including the final-review Process
        # button wiring - a test helper that skips it silently disconnects the
        # button from anything real.
        tab.process_here_button.clicked.connect(
            lambda _checked=False: window._process_after_final_review(window.batch.approved)
        )
...
def test_a_worker_failure_does_not_kill_the_process(qapp):
    ...
    # Wait on a clock, not on a fixed number of iterations. A busy CI runner
    # may not even have started the thread within fifty event-loop turns.
    deadline = time.time() + 20
    while time.time() < deadline and not seen:
        qapp.processEvents()
        time.sleep(0.02)

    assert seen, "the worker's failure never reached the UI"
    assert "MemoryError" in seen[0]
    runner.stop()
    parent.close()
...
def test_process_actually_writes_a_file(qapp, tmp_path, monkeypatch):
    ...
    # Final review is mandatory: the first Process press sends the user there
    # instead of writing anything. The panel's own button does the writing,
    # and only once at least one item has actually been decided there.
    tab = window.tabs.widget(0)
    assert tab.final_review, "final review was not enforced"
    tab.decide_group(tab.groups()[0], DecisionState.ACCEPTED)
    assert tab.process_here_button.isEnabled(), "button never enabled after a decision"
    tab.process_here_button.click()
...
def test_there_is_no_separate_second_pass_section(qapp, tmp_path):
    ...
    "Unlabelled" is different: those are values nothing could type, so nothing
    replaces them until the user says what they are.
    """
    window = _window(tmp_path, 1)
    tab = window.tabs.widget(0)
    assert set(tab.chips) == {"flagged", "unlabelled", "reviewed", "all", "kept"}
...
def test_the_type_picker_returns_a_real_enum(qapp):
    """Regression: adding an item crashed with 'str' has no attribute 'value'.

    PiiType subclasses str, so Qt stored the member as a plain string and
    handed a str back. Storing that broke the review list the moment it
    grouped candidates by type.
    """
    from app.detection.types import PiiType
    from app.ui.dialogs import _TypePicker, coerce_type

    picker = _TypePicker(PiiType.ORG_PRIVATE)
    chosen = picker.selected_type()
    assert isinstance(chosen, PiiType)
    assert chosen is PiiType.ORG_PRIVATE
    assert hasattr(chosen, "value")

    assert coerce_type("SSN") is PiiType.SSN
    assert coerce_type(PiiType.PERSON) is PiiType.PERSON
    assert coerce_type("not a type") is PiiType.UNCLASSIFIED_GROUP_VALUE


def test_adding_with_a_string_type_does_not_break_the_list(qapp, tmp_path):
    """The session must coerce whatever the UI hands it."""
    from app.detection.types import PiiType

    window = _window(tmp_path, 1)
    tab = window.tabs.widget(0)
    added = tab.session.add_manual_text("Annual Salary", pii_type="ORG_PRIVATE")
    assert added and all(isinstance(c.pii_type, PiiType) for c in added)

    tab.set_filter("all")          # this is what used to crash
    assert tab.cards
    window.close()


def test_final_review_hides_the_detection_list_and_shows_badges(qapp, tmp_path):
    """A last look at the document as it will be written."""
    window = _window(tmp_path, 1)
    tab = window.tabs.widget(0)

    assert tab.sidebar.isVisibleTo(tab)
    tab.set_final_review(True)
    assert not tab.sidebar.isVisibleTo(tab)
    assert tab.right_canvas.badges
    assert tab.view_mode == "redacted"
    assert "Back to review" in tab.final_button.text()

    tab.set_final_review(False)
    assert tab.sidebar.isVisibleTo(tab)
    assert not tab.right_canvas.badges
    window.close()


def test_dismissing_a_change_from_the_preview_puts_the_value_back(qapp, tmp_path):
    """The x on a change: that value stops being replaced."""
    window = _window(tmp_path, 1)
    tab = window.tabs.widget(0)
    tab.set_filter("all")

    target = tab.session.plan().targets[0]
    before = len(tab.session.plan().targets)

    tab.on_undo_requested(target.page_no, target.rect)

    after = tab.session.plan().targets
    assert len(after) < before, "the change was not removed"
    assert target.candidate_id not in {t.candidate_id for t in after}
    window.close()


def test_dismissing_an_unknown_region_is_harmless(qapp, tmp_path):
    window = _window(tmp_path, 1)
    tab = window.tabs.widget(0)
    before = len(tab.session.plan().targets)
    tab.on_undo_requested(0, (1.0, 2.0, 3.0, 4.0))
    assert len(tab.session.plan().targets) == before
    window.close()


def test_badges_are_drawn_for_applied_changes_only(qapp, tmp_path):
    from app.decisions.manager import DecisionState
    ...
    tab.set_final_review(True)
    tab._draw_applied_badges()
    applied = {t.rect for t in tab.session.plan().targets}
    drawn = {rect for view in tab.right_canvas.views.values() for rect, _k, _f in view._overlays}
    assert drawn <= applied, "a badge was drawn for something that will not change"
    window.close()


def test_final_review_is_mandatory_before_processing(qapp, tmp_path, monkeypatch):
    """The footer Process button must not write anything on its own - it
    redirects to final review, and only the panel's own button writes."""
    import app.ui.main_window as mw
    ...
    assert not tab.final_review
    window.process()
    assert tab.final_review, "Process did not enforce final review first"
    assert window.stack.currentWidget() is not window.processing, (
        "the footer button wrote the file without a final review pass"
    )
    window.close()


def test_the_final_review_sidebar_lists_every_detected_item(qapp, tmp_path):
    window = _window(tmp_path, 1)
    tab = window.tabs.widget(0)
    tab.set_final_review(True)

    rows = tab.final_list_layout.count() - 1  # minus the trailing stretch
    assert rows == len(tab.groups()), (rows, len(tab.groups()))
    assert "distinct value" in tab.final_summary.text()


def test_items_stay_editable_from_the_final_review_sidebar(qapp, tmp_path):
    from app.decisions.manager import DecisionState
    ...
    tab.decide_group(group, DecisionState.SKIPPED)
    assert tab.session.decisions.state(group.candidates[0]) is DecisionState.SKIPPED
    # The list refreshes to reflect the change without leaving the stage.
    assert tab.final_review


def test_the_final_sidebar_is_a_different_widget_from_the_working_sidebar(qapp, tmp_path):
    window = _window(tmp_path, 1)
    tab = window.tabs.widget(0)
    assert tab.sidebar is not tab.final_sidebar
    tab.set_final_review(True)
    assert not tab.sidebar.isVisibleTo(tab)
    assert tab.final_sidebar.isVisibleTo(tab)


def test_retyping_with_apply_all_finds_occurrences_never_detected(qapp, tmp_path, monkeypatch):
    """Reported: a name confirmed on one page stayed missed on another.

    Retyping with 'apply to every occurrence' checked used to match only
    against candidates that ALREADY existed - an occurrence the detectors
    never found at all had nothing to match against, so it stayed missed
    even after the user explicitly confirmed the value elsewhere.
    """
    import app.ui.main_window as mw
    from app.batch import BatchItem
    from app.decisions.manager import DecisionState
    ...
    c.showPage()
    c.setFont("Helvetica", 10)
    c.drawString(72, 720, "Prepared by Marisol Etxeberria for the file.")
    c.showPage()
    c.save()
    ...
    # Force the gap directly: remove the second page's occurrence entirely,
    # so the only way it can come back is through the retype cascade itself.
    occurrences = [c for c in tab.session.candidates if "Etxeberria" in c.text]
    assert len(occurrences) >= 1
    for extra in occurrences[1:]:
        tab.session.detection.candidates.remove(extra)
        tab.session.decisions.decisions.pop(extra.id, None)
    before = len([c for c in tab.session.candidates if "Etxeberria" in c.text])
    assert before == 1
    ...
    class AutoEdit(mw.EditDetectionDialog):
        ...
        def result_values(self):
            return (group.pii_type, "", True)

    monkeypatch.setattr(mw, "EditDetectionDialog", AutoEdit)
    tab.decide_group(group, DecisionState.EDITED)

    after = len([c for c in tab.session.candidates if "Etxeberria" in c.text])
    assert after > before, "the cascade search did not run on retype"
    window.close()


def test_a_failed_item_shows_its_real_error_as_a_tooltip(qapp, tmp_path):
    """Reported: the queue bar showed only '1 failed' with no way to see
    why - item.error already captured the real exception's type and
    message, it just never reached the GUI. Now it does, as a tooltip on
    both the failed tab itself and the summary bar."""
    from app.batch import ItemState

    window = _window(tmp_path, 1)
    item = window.batch.items[0]
    item.state = ItemState.FAILED
    item.error = "ValueError: something specific went wrong"

    window._refresh_queue()
    assert "1 failed" in window.queue_label.text()
    assert "something specific went wrong" in window.queue_label.toolTip()
    tooltip = window.tabs.tabToolTip(0)
    assert "something specific went wrong" in tooltip
    window.close()


def test_rebuild_final_list_never_calls_deletelater_on_none(qapp, tmp_path):
    """Real crash log (an older build): 'NoneType' object has no attribute
    'deleteLater' in rebuild_final_list, called from set_final_review.
    Already fixed with a takeAt()-then-None-guard; this exercises a rapid
    double rebuild - the exact shape that triggered it - rather than just
    reading the source."""
    window = _window(tmp_path, 1)
    tab = window.tabs.widget(0)
    tab.set_final_review(True)
    # A second, immediate rebuild in the same call stack - deciding an item
    # right after entering final review - is what originally raced against
    # deleteLater()'s deferred cleanup.
    tab.rebuild_final_list()
    tab.rebuild_final_list()
    window.close()
