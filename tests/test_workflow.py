...
def test_transformers_uses_module_collection_mode_not_collect_all():
    ...
    mechanism - module_collection_mode = 'pyz+py' - which a custom hook for
    the same module name likely shadowed rather than supplemented (one
    hook applies per module name, not a merge). This hook now adopts that
    mechanism directly instead of guessing at further alternatives, and
    "transformers" must not also appear in COLLECT_ALL - --collect-all and
    a hook's own settings are two different, potentially-conflicting paths
    to bundle the same package, and this took three real attempts to even
    get this far.
    """
    pytest.importorskip("PyInstaller")
    import importlib.util
    ...
    assert "transformers" not in build_module.COLLECT_ALL, (
        ...
        "a problem that has already taken three attempts"
    )
    ...
    assert hook_module.module_collection_mode == "pyz+py"
