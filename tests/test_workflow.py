...
def test_transformers_hook_copies_metadata_for_its_own_runtime_checks():
    ...
    real, complete pkgs_to_check_at_runtime list rather than fixing
    packages one at a time as each one surfaces in a future build.
    """
    pytest.importorskip("PyInstaller")
    import transformers.dependency_versions_check as check_module
    from PyInstaller.utils.hooks import copy_metadata

    real_checked = [p for p in check_module.pkgs_to_check_at_runtime if p != "python"]
    hook_source = open(
        WORKFLOW.parents[2] / "buildtools" / "hooks" / "hook-transformers.py"
    ).read()
    for pkg in real_checked:
        if pkg == "accelerate":
            continue  # this project does not install it; transformers itself
            # skips the check when absent, matching that choice deliberately
        assert pkg in hook_source, (
            f"'{pkg}' is checked by transformers' own dependency_versions_check.py "
            "at import time but is missing from hook-transformers.py's copy_metadata list"
        )
        # And prove copy_metadata actually succeeds for it, not just that
        # the name appears in the source.
        assert copy_metadata(pkg), f"copy_metadata found nothing for '{pkg}'"
