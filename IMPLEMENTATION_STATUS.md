...
**0.37.18** (bumped once, at the end of this work - not per intermediate edit,
per instruction).
...
## This pass (0.37.18): past spacy_alignments too - a genuinely different KIND of failure this time, and a real open question found along the way

A real macOS CI run got past BOTH the transformers/models crash and the
spacy_alignments gap, hitting a new failure of a different kind entirely
- not a packaging/file-discovery problem, but a spaCy pipeline factory
registry error: `ValueError: [E002] Can't find factory for
'curated_transformer'`.

**Investigated properly rather than pattern-matching to "another missing
package"**: installed en_core_web_trf-3.7.3 directly and read its actual
config.cfg. Every transformer-related component uses a spacy-curated-
transformers architecture (RobertaTransformer.v1,
LastTransformerLayerListener.v1, ByteBpeEncoder.v1) - never
spacy_transformers' own "transformer" factory at all. `pip show
en-core-web-trf` confirms spacy-curated-transformers is a real, declared
dependency of the model wheel itself, auto-installed transitively, never
explicitly bundled. Added spacy_curated_transformers, curated_transformers,
and curated_tokenizers (a native, compiled package - same class of gap as
spacy_alignments) to COLLECT_ALL. Verified directly, not assumed: with
these available, `spacy.load("en_core_web_trf", ...)` actually loads and
produces correct entities.

**A genuinely important, deliberately UNACTED-ON finding along the way**:
this raises a real question about whether spacy_transformers (and the
whole packaging saga around bundling HuggingFace transformers for
spaCy's sake) was ever actually necessary for this model - its own
factory does not appear anywhere in en_core_web_trf's pipeline. Did NOT
act on this now: GLiNER genuinely needs bare `transformers` directly for
its own tokenizer, so it cannot be removed from the project regardless,
and whether spacy_transformers specifically is safe to drop is a real,
separate question deserving its own dedicated, careful investigation -
not a change to bundle into an already-delicate build fix. Written up in
CLAUDE.md as a flagged, open question for later.

One regression test. Full suite: **342 passed, 0 failed** - the first
fully clean run of the whole transformers-packaging investigation (and
notably, the previously-skipped self-test-function test now genuinely
passes too, since installing en_core_web_trf here to verify this fix
left it actually available).
