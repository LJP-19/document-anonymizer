...
**PySide6 crashes during widget teardown.** Running the GUI test suite in one
process, roughly 40% of runs end with SIGBUS or SIGABRT *after* every assertion
has passed. It happens inside Qt while destroying widgets, and it is unresolved.

What was ruled out along the way, and fixed properly because each was a real
bug: a QThread destroyed while running, a debounce timer firing into a destroyed
tab, render callbacks landing on freed widgets, and - the one that mattered
most - MuPDF being called from several threads at once, which would have crashed
users on large documents and is now serialised behind a single lock.

Until it is understood, CI gates the build on the engine suite (`-m "not gui"`)
and runs the GUI suite as reporting-only. That is a deliberate trade: the
correctness of what gets redacted is fully gated; a Qt shutdown fault does not
block shipping.
