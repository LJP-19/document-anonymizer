...
| Automatic re-scan of the transformed output, merged into one review list | done |
| Manual review: search, pseudonymize or black out | done |
| Manual review: drag to draw a blackout area | done |
| Page-level context windows for the model | done |
| Lookalike character normalization | done |
| Table column typing from header rows | done |
| Form text protection: headers, footers, instructions, headings | done |
| Type-or-skip: never replace a value nothing could identify | done |
| Careful mode: high-confidence detections only | done |
| Final review stage with dismissable changes | done |
| Final review sidebar: every item, still editable | done |
| Final review mandatory before Process | done |
| Propagation exempt from repeated-position form-text veto | done |
| Standalone joint-name detection, independent of labels | done |
| Final safety pass: labels never exposed by widening/propagation | done |
| Careful mode keeps label-confirmed (GROUP) evidence | done |
| Address spelling variants share one pseudonym | done |
| Presidio proposal layer with label-context boosting (optional) | done |
| Evidence-based adjudication (confirmed/probable/unresolved, per-type thresholds) | done |
| PII type coverage matrix, generated and CI-checkable | done (47/47 types) |
| Precision/recall benchmark harness with golden fixture corpus | done |
| Presidio paragraph-context pass for unlabelled prose blocks | done |
| Font weight/slant (bold/italic) and baseline preserved on replacement | done |
| Geometric (bounding-box) label exclusion, independent of Line objects | done |
| Boilerplate vocabulary: voucher/cut-line fragments | done |
| Tax-form lexicon veto (IRA/rollover/etc.) for name-shape matching | done |
| Cascade search closes the gap in retype-and-apply-to-all | done |
| Union-find entity graph: co-occurring fields share a reviewed mark | done |
| PIILeakError: exception wrapper around the existing leak sweep | done |
| Deterministic rules match regardless of case (opt-out per rule) | done |
| City/state address matches without a comma (condensed mailing blocks) | done |
| Model proposes, detectors confirm and type | done |
| Token-level name mapping: one person, one identity, any format | done |
| Joint names share a surname by construction | done |
| Identifiers matched in any format, keyed by digits | done |
| Replacement plausibility checks before writing | done |
...
The Qt runtime, PyMuPDF, spaCy and its model, ONNX Runtime and the GLiNER
weights (46 MB). A machine receiving the installer needs no Python, no models
and no network, ever.

Expect roughly **220 MB**. The Qwen audit model is no longer included: it
proposed labels, headings and figures as often as it found real PII. Build with
`--with-llm` if you want it back. A build near 220 MB is missing the audit model; the
build now refuses to publish in that state rather than shipping quietly reduced
capability. `buildtools/verify_bundle.py` checks the produced bundle, not the
source tree, and fails the build if anything large is absent.

To build deliberately without it: `python buildtools/build.py --no-llm`.
...
# Optional: the LLM audit pass (~1.1 GB). Skip it and detection still runs.
