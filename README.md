...
| LLM audit pass: finds misses on uncertain pages | done |
| LLM adjudication: reviews every proposed redaction | done |
...
| Joint names keep a shared surname | done |
...
Everything. The `.exe` and `.dmg` contain the Qt runtime, PyMuPDF, spaCy and its
model, ONNX Runtime, the GLiNER weights (46 MB), llama.cpp and the Qwen audit
model (1.1 GB). A machine receiving the installer needs no Python, no models and
no network, ever.

Expect roughly **1.5 GB**. A build near 220 MB is missing the audit model; the
