...
## SUPERSEDED - see "en_core_web_trf is the production default" below

An earlier version of this file argued en_core_web_sm should stay the
default and en_core_web_trf should be opt-in only via requirements-trf.txt.
That file has since been deleted - its contents were folded directly into
requirements.txt once trf became the mandatory default. Do not resurrect
either the file or that argument; see the superseding entry later in this
file for why and what actually ships now.

## Filename anonymization routes name tokens through NameRegistry, never a placeholder

`safe_output_name()` in session.py must map a bare name token in the filename
to the SAME per-token pseudonym the document body uses (via
`self.registry.names.pseudonym(token)`), never to a generic placeholder like
"REDACTED" - that was a real, shipped bug: "Lance" in a filename became the
literal word "REDACTED" instead of whichever given name "Lance" became
inside the document. A name-shaped filename token with no matching document
candidate still gets a stable pseudonym via the same registry (spec: filename
PII need not occur in the PDF body), but the stopword list guarding this must
stay wide - filenames are full of ordinary title-cased English words (Report,
Final, Files, Documents...) that the lone-surname heuristic is permissive
enough to mistake for names on a single bare token.
...
## en_core_web_trf is the production default now - REVERSED from earlier guidance

Earlier entries in this file argued against trf as the default. That guidance
was correct given the information at the time, but the person made this call
twice more, explicitly addressing the performance concern with a real
tiered-execution architecture. Continuing to refuse a third time would have
been substituting my judgment for theirs on their own repository - I
implemented it as the actual default, not merely an opt-in. Do not silently
revert this back to en_core_web_sm on the strength of the OLD entries below;
if performance becomes a live problem again, that is a new decision for the
person to make with current evidence, not a reason to reach for stale
reasoning already superseded once.

Real, measured cost of this: ~1.8 GB installed (torch CPU wheel +
spacy-transformers + the model), 5-6s one-time load per process (cached via
lru_cache(maxsize=1) - already existed, verified still correct), ~0.18s
inference per short block. `spacy.require_cpu()` is called explicitly in
load_nlp() - GPU must never even be attempted, not just left unused.

## en_core_web_trf tokenizes differently than en_core_web_sm - do not assume parity

Switching the default model surfaced two real, model-specific behaviour
differences neither prior testing against sm ever hit:

1. trf's entity span for a name split across a line break can include a
   trailing newline INSIDE the span ("Marisol\n") where sm did not. Any code
   converting a model's character span to Document geometry must trim
   leading/trailing whitespace from the span FIRST (see the trim in
   detect_ner's loop) - assuming a model's boundary is exactly the visible
   text is not safe across models.
2. trf segments a name split across a line break into TWO separate
   single-token entities, where sm apparently returned ONE combined
   two-token span. `_implausible_person`'s blanket single-token rejection
   (added to filter "Daytime"/"Preparer") silently ate this until a second,
   independent signal (`looks_like_a_lone_surname`) was added to let a
   single token through when it genuinely looks name-shaped. If another
   model swap happens later, re-verify this function against a name split
   across a line break specifically - it is the exact case this class of
   bug hides in.

## Tiered NER execution: skip blocks with nothing left for the model to find

`_fully_covered_or_no_letters` in ner.py skips calling the transformer on a
block once money/percent matches and FORM_VOCABULARY words are stripped from
it and nothing alphabetic remains - a pure financial table row, a page
footer. Verified this costs zero recall (same candidate count with and
without tiering on a 30-row financial-table fixture) while genuinely skipping
those rows. Do NOT widen this check casually - FORM_VOCABULARY has grown to
include many ordinary English words for narrow, unrelated fixes earlier in
this project's history (the "Need to Keep" fix, the tax-lexicon fix), and
reusing it as a "should we even run the model" gate is a much more dangerous
use of that list than its original purpose (rejecting a name-shape match on
one line). Test any change here against an ordinary prose paragraph, not
just a financial table, before trusting it.

## verify_bundle.py must actively REJECT en_core_web_sm/lg, not just require trf

Two separate checks, not one: REQUIRED now lists en_core_web_trf/spacy-
transformers/torch, and a SEPARATE loop actively fails the build if
en_core_web_sm or en_core_web_lg are present at all, even alongside trf.
Absence from the required list is not the same guarantee as active
rejection - a bundle could satisfy "trf is present" while still shipping sm
too "just in case," which the spec explicitly forbids.

## This sandbox has ~3.9 GB RAM - real full-suite runs against live trf were not completed here

The benchmark (18 fixtures) and a representative ~160-test subset ran clean
against REAL en_core_web_trf inference (not the sm speed-override). Running
the full ~320-test suite against live trf in THIS environment hit a hard
OOM kill partway through, reproducibly, at roughly the same point each time.
Traced the obvious causes first (duplicate model loading, a caching gap) and
ruled both out - the cache was already correct, Presidio already reuses the
shared instance. This is very likely this sandbox's RAM ceiling rather than
a shipped-application defect, since a typical office machine has
meaningfully more headroom, but it was never independently confirmed on a
real, larger machine within this engagement. If a user reports memory
pressure on an ordinary machine after this change, this note is the
starting point, not a reason to be surprised.


## item.error is captured everywhere it can happen - it must also be DISPLAYED

A user reported "1 failed" at the bottom of the queue with no way to find
out why. item.error already captured the real exception (type + message) at
both places a batch item can fail (analysis in batch.py, and process/export/
verify) - it was simply never READ anywhere in the GUI, only written to
app.log (~/.document-anonymizer/app.log). Fixed by surfacing it as a
tooltip on the failed tab AND on the queue summary label itself
(_refresh_queue in main_window.py). If a NEW failure path is added anywhere
(a new except block setting ItemState.FAILED), it must also set item.error
with a real message, and that message must reach one of these two tooltips
- capturing an error without ever displaying it is the same practical
failure as not capturing it at all.


## numpy must stay pinned below 2.0 - a real, confirmed root cause, not a guess

`numpy<2` in requirements.txt exists because a genuinely fresh resolve pulls
numpy 2.5.3 (spacy's own constraint is only numpy>=1.19.0), and thinc/blis
(spaCy's compiled Cython internals) were built against numpy 1.x's C-API,
which numpy 2.0 broke. This produced a real, reported crash in the packaged
app: every document analysis failed instantly with `SystemError: <class
'ImportError'> returned a result with an exception set` - a well-known
signature of exactly this class of ABI mismatch. Confirmed by direct
evidence, not inference: the real CI install log showed numpy resolving to
2.5.3; a genuinely fresh venv with the `numpy<2` pin resolves to 1.26.4 and
imports/runs en_core_web_trf + torch together with no error. Do not remove
or loosen this pin without re-testing en_core_web_trf + torch + thinc/blis
together in a GENUINELY FRESH environment first - a sandbox that already has
an older numpy installed from unrelated earlier work will not catch a
regression here, which is exactly how this shipped in the first place.


## Every CI smoke test used unfrozen Python - none could catch a PyInstaller bundling bug

`python -m app.cli` in CI runs the runner's own, normal, unfrozen Python
interpreter, with every package's compiled binaries properly isolated in
their own site-packages directory. That structurally cannot detect a bug
that only exists in the FROZEN bundle - PyInstaller merges every package's
binary files into one flat directory, and if two packages (torch and numpy,
most plausibly, though never fully confirmed) each vendor their own copy of
an overlapping shared library, the frozen build can collide in a way a
normal venv never would. This is very likely how a real, reported
`SystemError: <class 'ImportError'> returned a result with an exception
set` shipped: every existing smoke test passed cleanly, right up until the
actual .exe tried the same operation.

`app/main.py`'s `--self-test` flag and the CI steps that invoke it
(`DocumentAnonymizer.exe --self-test` / `DocumentAnonymizer.app/Contents/
MacOS/DocumentAnonymizer --self-test`, immediately after each platform's
Build step) are the fix: the FIRST point in the whole workflow that
actually executes the shipped artifact, not just builds and uploads it.
Keep this invocation immediately after Build on both platforms - do not
let a future refactor of the workflow move it later or drop it, since
nothing else in CI can catch this class of bug.

## test_self_test_mode_exists_and_reports_failure_clearly calls the function in-process

Deliberately not via a subprocess. A second, fully separate interpreter
reloading the whole torch/spacy/trf stack on top of an already-heavy
pytest run is real resource contention in a constrained sandbox - a fact
about the TEST's footprint, not about what it verifies. The real
frozen-executable self-test always runs as its own dedicated CI step, never
nested inside a long test run, so testing the function in-process here does
not weaken what CI actually checks.


## --self-test's output must never depend on console attachment working

A --windowed PyInstaller build (which this project's build IS: confirmed
directly in the CI build log) is a GUI-subsystem Windows executable with NO
console attached, ever - not "attached but empty," genuinely absent. print()
output from such a process goes nowhere when run from cmd.exe/PowerShell,
and the terminal returns to the prompt with no visible error either way. A
real user ran `DocumentAnonymizer.exe --self-test` and got exactly this:
blank output, no crash, nothing. My own testing never caught it because
every test ran `python -m app.main --self-test` directly - ordinary console
Python, nothing like what --windowed actually does. Same class of gap as
the unfrozen-CLI-smoke-test issue two passes earlier: my test environment
kept not matching the real frozen behaviour.

Fixed two ways, deliberately redundant rather than picking one:
1. `_attach_console_for_self_test()` calls `AttachConsole(-1)` (the standard,
   documented Windows fix) so print() becomes visible when a console
   happens to exist - best-effort, silently skipped on any failure or on
   non-Windows platforms.
2. Every exit path writes the full result - stdout lines AND the traceback
   on failure - to `~/.document-anonymizer/self-test-result.txt`,
   unconditionally, regardless of whether console output worked at all.

Do not remove #2 on the theory that #1 makes it redundant. #1 is a nicety
that depends on Windows console API behaviour I cannot verify from this
sandbox; #2 is the one guarantee that survives no matter what the terminal
does. Any future change to this function must keep writing the result file
on every exit path, success and failure both.


## Propagation must not transform a known person into a different entity's name

Word-boundary matching alone is not enough for propagate(): "LJP" inside
"LJP Manufacturing Inc." passes a word-boundary check cleanly (it IS a
genuine standalone token), but propagating the known person "LJP" onto
that occurrence produces "[pseudonym] Manufacturing Inc." - a real,
reproduced failure, and the exact canonical example from the original
spec. `ORG_SUFFIX_AFTER` in entities_pass.py checks what immediately
follows a candidate match; if it's an organizational suffix (Inc., LLC,
Corp., Manufacturing, etc.), that occurrence is never propagated as the
person at all - not demoted, skipped outright, per the spec's own wording
("should not automatically transform ... into the PERSON entity"). Scoped
to PiiType.PERSON only - an SSN or address has no equivalent "this might
be a different thing" signal from what follows it. Do not weaken this to
a confidence penalty; the whole point is that this occurrence is not the
same person, full stop.

## The verifier must know that a value can legitimately survive more times than were redacted

Fixing the guard above exposed a second, separate bug: the verifier's leak
checks (`_check_originals_absent`, `_check_group_completeness`,
`_check_partial_redaction`, `_check_group_residue`) all did a naive
whole-document substring/count search assuming EVERY occurrence of an
accepted value must vanish. That's wrong whenever an occurrence was
correctly never a candidate at all (the org-suffix case above). Fixed by
computing `plan.expected_residual_counts` once, at build_plan() time, from
the ORIGINAL document (total occurrences of a value minus how many became
targets), and comparing against that instead of assuming zero.

The token-level checks (`_check_partial_redaction`, `_check_group_residue`)
need the more careful version of this: a token surviving standalone is
only excused when EVERY one of its standalone occurrences is accounted for
by an INTACT full-phrase occurrence of the complete original value/line,
itself within the proven residual (`min(allowance, full_phrase_count)`).
A token appearing more often alone than the full phrase appears AT ALL
means at least one occurrence broke off from an intact phrase - exactly
the "John redacted, Smith survives alone" partial-redaction failure this
check exists to catch - and that must always still fail, regardless of any
allowance existing elsewhere in the same document. Do not simplify this
back to "allow if the value has only one token" - that was the first,
too-narrow version of this fix, and it wrongly flagged the far more
realistic multi-word case ("Marcus Feldman Manufacturing Inc.") as a
failure on a document that was actually handled correctly.

## A short, all-caps token is a bad choice for testing subject-identification logic

`en_core_web_sm` can initially classify a short all-caps string like "LJPS"
as ORG_PRIVATE rather than PERSON, with a LATER pass correcting it - and
identify_subjects()/propagate() run BEFORE that correction, so they
legitimately see nothing to propagate. This is a real, separate
pipeline-ordering fact, not a bug in either the org-suffix guard or the
residual-count fix. Any future test of subject-identification/propagation
should use an unambiguous full name (e.g. "Marcus Feldman"), not a short
acronym-shaped placeholder, or it will fail unpredictably depending on
which detector happens to type the seed candidate first - which is exactly
what made this take three attempts to pin down.


## transformers must be in COLLECT_ALL - a dependency of a bundled package is not itself bundled
...
package directory with os.listdir() at import time - a dynamic filesystem
read no static import-analysis can trace - so leaving it out of
COLLECT_ALL is invisible until the model actually tries to load, which
none of the unfrozen CI smoke tests can ever catch (they use the CI
runner's own properly-installed transformers, not the frozen bundle's).
This produced a real crash - FileNotFoundError on
Contents/Frameworks/transformers/models/__init__.pyc - on the very first
real run of the --self-test step added specifically to catch exactly this
class of bug. It worked: caught in CI, before it ever reached a person.

Fixed two ways, matching the "the self-test itself is not the whole
guarantee" lesson: `transformers` added to COLLECT_ALL in build.py (the
actual fix), AND `verify_bundle.py`'s REQUIRED list now checks for
`transformers/models/__init__.py*` specifically - the exact file that was
missing - so this defect is caught immediately after packaging, before
paying for a full model-load in the self-test step. If a future dependency
does its own os.listdir()-based dynamic package scanning the same way,
the same two-part pattern (COLLECT_ALL entry + a REQUIRED check for the
specific file its runtime needs) is the right fix, not just one or the
other.


## chardet must be excluded from the build entirely

spacy/__init__.py unconditionally imports its OWN cli/download subsystem
(the `spacy download <model>` command-line tool - never invoked by this
app, since the model ships bundled and is never fetched at runtime), which
pulls in `requests`, which optionally tries `chardet` before falling back
to `charset_normalizer`. requests' own fallback only catches plain
ImportError - but a real, reported crash on the second real Windows CI run
showed chardet breaking with the SAME SystemError signature as the earlier
numpy ABI issue when frozen (`SystemError: <class 'ImportError'> returned
a result with an exception set`), which is NOT an ImportError and escapes
the fallback, breaking `import spacy` entirely.

Verified directly (not just inferred from the traceback): with chardet
made genuinely unimportable, `requests` correctly falls through to
`charset_normalizer` and `spacy` imports cleanly end to end. Confirmed
nothing else installed imports chardet unguarded either. `chardet` is now
in build.py's `EXCLUDE_MODULES` and passed via `--exclude-module`; do not
add it back "just in case" - it is provably unused, and its presence is
what caused the crash. verify_bundle.py's REQUIRED-rejection list also
checks it is genuinely absent, the same way it already rejects en_core_web
sm/lg, so a future accidental re-inclusion is caught immediately after
packaging rather than only surfacing again as a frozen-build-only crash.

This is the third distinct real crash surfaced by real CI runs of the
--self-test step (numpy ABI mismatch, missing transformers package files,
now this). Each was invisible to every unfrozen CLI smoke test and would
have shipped silently without --self-test actually executing the frozen
artifact. When a NEW frozen-only import crash appears with this same
`SystemError: <class 'ImportError'> returned a result with an exception
set` signature, the pattern to check first is: (1) is this a real ABI/
version issue (like numpy), or (2) is the crashing package actually
unused and excludable (like chardet) - check for a graceful, already-
existing fallback in the code path before assuming a version pin is
needed.


## --collect-all does NOT keep a package's .py/.pyc files as loose files - a real regression that shipped twice

Adding "transformers" to COLLECT_ALL (v0.37.10) was NOT sufficient - the
exact same crash (FileNotFoundError on
Contents/Frameworks/transformers/models/__init__.pyc) happened again on the
next real macOS CI run. PyInstaller's own docs are explicit about this:
collect_data_files() (what --collect-all uses under the hood) excludes
.py/.pyc files BY DEFAULT; they normally get compiled into the PYZ archive
instead of staying as loose files, unless include_py_files=True is passed
explicitly. transformers.models does its own os.listdir() scan of its
package directory at import time - it needs real loose files, not just
importable code. Verified this directly against the real installed
package (collect_data_files("transformers", include_py_files=True)
genuinely returns the missing file) before writing the fix.

Fixed with `buildtools/hooks/hook-transformers.py`, wired in via
`--additional-hooks-dir` in build.py. Do not revert this to a plain
COLLECT_ALL entry on the theory that it's redundant - it demonstrably is
not; that was tried first and failed on a real CI run. If a FUTURE package
does its own filesystem introspection of its own directory the same way,
this hook-with-include_py_files=True pattern is the fix, not another
--collect-all attempt.

## verify_bundle.py existed but was never actually run in CI - a second real gap

The REQUIRED-list check for the transformers/models file (added in
v0.37.10) would have caught the missing-py-files regression immediately
after packaging, for a fraction of the self-test's cost. It did not,
because verify_bundle.py was never wired into build-release.yml at all -
it sat in the repo, complete and correct, doing nothing. Now runs as its
own step on both platform jobs, immediately after Build and before
Self-test. Keep this ordering: cheap file-presence checks before the
expensive full model-load, on both platforms, every time a new build step
is added to this workflow.


## Tests that import PyInstaller directly must skip cleanly, not fail, when it is absent

PyInstaller is a build-time-only tool, deliberately not in
requirements-dev.txt - the Linux "test" CI job runs the full pytest suite
without ever installing it, correctly, since that job never builds
anything. A test that does `from PyInstaller.utils.hooks import ...` at
module or test level without `pytest.importorskip("PyInstaller")` first
will hard-fail there, not because anything is broken, but because the
test itself assumed an environment it does not actually run in. This
happened for real: the very first genuine CI run of the full suite after
the hook-transformers.py fix failed on exactly this, with the real fix
(the hook, the build.py wiring, the CI wiring) all correct and unaffected.
Any future test that needs PyInstaller's own API to verify something
about the build (as opposed to just reading build.py's source as text,
which needs no import) must guard with importorskip.


## Tests asserting exact path strings must normalize separators - Windows CI catches what a Linux sandbox cannot

`collect_data_files`'s dest paths are built via `str(pathlib.Path...)`,
which is platform-native: backslash-separated on Windows, forward-slash on
POSIX. A test written and verified only in this Linux sandbox
(`dest == "transformers/models"`) hardcoded the POSIX form and could never
have passed on the real Windows CI job, regardless of whether the actual
fix worked - this happened for real, on the second genuine full-suite
Windows run. Fixed by normalizing separators explicitly
(`dest.replace("\\", "/")`) before comparing, and - since this sandbox
cannot literally run on Windows to re-verify - added a second, standalone
test that exercises the same normalization logic directly against a
simulated Windows-style path string, so the fix is actually verified here
rather than merely asserted to be correct. Any future test asserting an
exact path string from a PyInstaller/pathlib-derived value needs this same
normalization, or the same Linux-passes-but-Windows-fails gap repeats.


## transformers also needs its own runtime dependency METADATA bundled, not just its source files

A SEPARATE gap from the include_py_files=True fix, surfaced only once that
one was resolved: transformers checks its own runtime dependencies at
import time (transformers/dependency_versions_check.py,
pkgs_to_check_at_runtime) by reading each package's INSTALLED METADATA
(.dist-info) via importlib.metadata.version() - a different thing entirely
from the package's source files. collect_data_files()/--collect-all do not
bundle .dist-info folders; only copy_metadata() does. A real macOS CI run
proved this: PackageNotFoundError for "regex", the first package in that
check list, immediately after the include_py_files fix got past the
earlier FileNotFoundError.

Read transformers/dependency_versions_check.py directly to get the real,
complete pkgs_to_check_at_runtime list (tqdm, regex, requests, packaging,
filelock, numpy, tokenizers, huggingface-hub, safetensors, pyyaml -
excluding "python", not a real package, and "accelerate", which this
project does not install and transformers itself skips checking when
absent) rather than fixing one missing-metadata package at a time across
N future builds. hook-transformers.py now calls copy_metadata() for all
of them. If transformers' own runtime dependency list ever changes, re-
read dependency_versions_check.py directly rather than guess at the diff.

## A test's platform-specific path assumption is a real category of bug, not just style

collect_data_files() builds its returned dest paths via
str(pathlib.Path(...).relative_to(...)), which renders with the OS-native
separator - backslash on Windows, forward-slash on POSIX. A test that
hardcodes a forward-slash comparison will genuinely fail on a real Windows
CI run regardless of whether the thing it is testing actually works -
this happened for real and briefly looked like the underlying fix itself
had failed on Windows, when it was only the test's own assertion that was
wrong. Any test asserting on a path PyInstaller's hook utilities return
must normalize separators before comparing (dest.replace("\\\\", "/") or
equivalent), and where practical, verify the normalization logic directly
against a simulated Windows-style string (not just against whatever this
sandbox's own OS happens to produce), since this sandbox cannot reproduce
a real Windows path separator to catch this by running the test locally.


## Two independently-derived transformers packaging fixes both failed identically on real CI - the community-maintained hook was the actual answer

Verified-correct logic in isolation (collect_data_files(..., include_py_
files=True) genuinely returns the missing file when called directly) did
NOT translate into correct behavior in a real PyInstaller build - the
EXACT same FileNotFoundError on transformers/models/__init__.pyc
reproduced identically on both a real Windows and a real macOS CI run
after that fix shipped. The actual cause: pyinstaller-hooks-contrib (a
REQUIRED, hard dependency of pyinstaller itself - confirmed via `pip show
pyinstaller`) already ships its own hook-transformers.py, using a
different, PyInstaller-native mechanism this project's custom hook was
not using: `module_collection_mode = 'pyz+py'`, a module-level variable
PyInstaller's Analysis pass reads directly, documented by PyInstaller
itself as "the preferred way" over collect_data_files(...,
include_py_files=True). A custom hook for the same module name almost
certainly SHADOWED the contrib one - PyInstaller applies one hook per
module name, not a merge - meaning the custom hook replaced an
already-correct, community-maintained mechanism with a narrower, unproven
one.

hook-transformers.py now sets `module_collection_mode = "pyz+py"`
directly, keeps the include_py_files data collection as a harmless
supplement, and reuses the contrib hook's OWN broader metadata-copying
technique (checking transformers' full dependency table dynamically via
get_module_attribute/is_module_satisfies, not a hardcoded package list).
"transformers" was REMOVED from COLLECT_ALL entirely - --collect-all and
a hook's own settings are two different, possibly-conflicting paths to
bundle the same package, and after three real attempts this is not a
variable worth leaving unexamined.

**The general lesson**: when a THIRD-PARTY hooks-contrib package already
exists for a library, check what it does FIRST, before writing a custom
hook - a custom hook for the same module name does not supplement an
existing one, it silently replaces it. Verifying a fix's logic in
isolation (in a plain Python call) is necessary but was proven NOT
sufficient here - it does not prove the fix survives a real PyInstaller
Analysis pass with all its own hook-precedence and merging rules, which
this sandbox cannot reproduce.


## 'pyz+py' does NOT force loading from the external file - PYZ staying set means the archive still wins

Verified this against PyInstaller's own loader source
(PyInstaller/loader/pyimod02_importers.py, `_fixup_frozen_stdlib`), not
just inferred from a traceback: 'pyz+py' = PYZ|PY flags together - it adds
an external loose .py copy ALONGSIDE the archived one, it does not replace
it. The loader still imports the archived copy by default, and for such a
module synthesizes `__file__` as
`os.path.join(sys._MEIPASS, *orig_name.split('.')) + '.pyc'` - the
loader's own comment says this exists "to be consistent with our
PyiFrozenLoader" - a conventional path that need not correspond to a real
file on disk, regardless of whether an external .py copy exists
elsewhere under a different path. transformers.models reads
`_file = globals()["__file__"]` and hands it to `os.listdir()`; as long as
PYZ stays set for the module, this stays broken no matter how many loose
copies also get bundled elsewhere.

'pyz+py' reproduced the IDENTICAL FileNotFoundError on a real macOS CI
run, proving this precisely. `module_collection_mode = "py"` (PY alone,
no PYZ) is the actual fix: with no archived alternative at all, the
loader has no choice but to use the real, on-disk external file, which
has a genuine, resolvable __file__.

**The general lesson, worth being explicit about**: adopting a mechanism
because it looks like "the preferred/documented approach" or because a
community package uses it is not the same as verifying it actually does
what you need for YOUR specific symptom. `module_collection_mode` has
multiple distinct values (pyz, pyc, py, pyz+py) with materially different
runtime behavior, not one "correct" setting - read the loader/runtime code
that actually produces the failing symptom before picking one, rather than
picking whichever option sounds closest to right.


## spacy_alignments must be in COLLECT_ALL - real progress, and a real, much simpler bug it revealed

The module_collection_mode="py" fix for transformers (previous entry)
worked: a real macOS CI run got completely past the long-running
transformers/models os.listdir() crash for the first time and hit a
different, unrelated, much simpler problem instead - ModuleNotFoundError
for spacy_alignments. spacy_transformers declares it as a real, required
dependency (`pip show spacy-transformers` lists it under Requires), but
it is a native-compiled package (Rust-backed, a single .so extension with
no further pip-declared dependencies of its own - confirmed directly) that
was never in requirements.txt (only installed transitively) or in
COLLECT_ALL. PyInstaller's static import analysis cannot trace into a
compiled extension's own runtime imports to discover a dependency like
this on its own - the same general class of gap as spacy_alignments
itself, torch, and transformers, just simpler to fix since it has no
onward dependency-chain complications of its own. Added directly to
COLLECT_ALL; no hook needed, unlike transformers.


## en_core_web_trf-3.7.3 actually uses spacy-curated-transformers, not spacy_transformers' own architectures

Verified directly against the real installed model's config.cfg, not
assumed: every transformer-related pipeline component in en_core_web_trf-
3.7.3 uses a spacy-curated-transformers architecture
(RobertaTransformer.v1, LastTransformerLayerListener.v1,
ByteBpeEncoder.v1, WithStridedSpans.v1) - never spacy_transformers' own
"transformer" factory. `pip show en-core-web-trf` confirms this: its own
Requires line lists spacy and spacy-curated-transformers, nothing else
transformer-related. A real macOS CI run proved this directly: after the
transformers/models and spacy_alignments gaps were both fixed, the very
next failure was `ValueError: [E002] Can't find factory for
'curated_transformer'`.

spacy_curated_transformers, curated_transformers, and curated_tokenizers
are now in COLLECT_ALL - the latter is a native, compiled package
(C++/Cython .so extensions, confirmed directly) matching the same class
of gap as spacy_alignments; curated_transformers is included defensively
(pure Python, but has its own model/tokenization subpackages, and enough
packages in this exact dependency tree have turned out to need explicit
collection that this is not worth treating as a safe assumption).
Verified directly that the model actually loads and produces correct
entities once these are available, not just that they are importable.

## A genuinely open question, deliberately NOT acted on yet: is spacy_transformers/transformers still needed at all for en_core_web_trf?

Given the finding above, spacy_transformers' own "transformer" factory
does not appear to be used by en_core_web_trf-3.7.3's pipeline at all -
raising the real possibility that spacy_transformers (and, by extension,
much of the packaging complexity around bundling HuggingFace transformers
for spaCy's sake specifically - the numpy ABI issue, the transformers/
models os.listdir() saga, the metadata copying) was never actually
necessary for THIS model to function. This was NOT acted on: GLiNER
(app/detection/gliner.py) directly imports `transformers.AutoTokenizer`
for its own, separate, genuine need - so bare `transformers` cannot be
removed from the project regardless. Whether `spacy_transformers`
specifically (as opposed to bare `transformers`) could be dropped
entirely is a real, open question worth a careful, DEDICATED
investigation of its own - not something to attempt as a rider on top of
an already-delicate packaging fix. If this is ever pursued: test whether
`spacy.load("en_core_web_trf", ...)` still works with spacy_transformers
genuinely uninstalled (not just untested), then decide whether removing
it from requirements.txt/COLLECT_ALL is safe.
