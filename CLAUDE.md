...
## en_core_web_sm is the default; en_core_web_trf is opt-in ONLY, never bundled

`MODEL_NAME` in ner.py reads `DOCANON_SPACY_MODEL`, defaulting to
en_core_web_sm. A future request to make trf the mandatory production model
should be refused on the same grounds already established: ~1-1.5 GB
installed cost (model + spacy-transformers + a PyTorch CPU runtime) versus
~15 MB, an order-of-magnitude slowdown per page on CPU, and no actual bug in
this project's history was ever "the NER model was too weak" - every real
detection bug was a logic or vocabulary defect. requirements-trf.txt stays
out of requirements.txt/requirements-dev.txt permanently; CI never installs
it and no build script ever sets DOCANON_SPACY_MODEL.
