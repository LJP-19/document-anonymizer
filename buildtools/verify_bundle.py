...
REQUIRED = [
    ...
    # spacy_transformers depends on this, but PyInstaller bundling a
    # package's traced imports into the PYZ archive is not the same as
    # preserving its loose files on disk. transformers.models scans its
    # OWN package directory with os.listdir() at import time - a dynamic
    # filesystem read no static import-analysis catches - so its absence
    # here is invisible until the model actually tries to load. This exact
    # gap produced a real crash the first time the --self-test step ran
    # for real in CI (FileNotFoundError on
    # Contents/Frameworks/transformers/models/__init__.pyc); checking for
    # it explicitly here catches it immediately after packaging, before
    # paying for a full model-load in the self-test step.
    ("transformers package", "**/transformers/models/__init__.py*"),
...
def check(require_llm: bool) -> list[str]:
    ...
    # shipped bundle, not even alongside trf "just in case." chardet must
    # never be present either: spacy's own import chain drags it in as an
    # optional, unused dependency of requests, and a real, reported crash
    # showed chardet breaking with a SystemError when frozen that requests'
    # own fallback-to-charset_normalizer logic cannot catch (it only catches
    # plain ImportError). build.py excludes it explicitly; this check
    # catches a regression if that exclusion is ever accidentally removed.
    for forbidden_name, forbidden_pattern in (
        ...
        if any(root.glob(forbidden_pattern)):
            problems.append(
                ...
                if forbidden_name != "chardet"
                else "chardet is present in the bundle - it must be excluded "
                "(build.py's EXCLUDE_MODULES), not merely unused"
            )
