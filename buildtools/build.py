...
COLLECT_ALL = [
    ...
    "spacy_loggers",
    "spacy_transformers",
    # spacy_transformers depends on this, but depending on a package is not
    # the same as PyInstaller bundling its loose files. transformers.models
    # scans its OWN package directory with os.listdir() at import time - a
    # dynamic filesystem read, not a statically traceable import - so it
    # needs its real files preserved on disk, not just compiled into the
    # PYZ archive via ordinary import-following. Its absence here produced
    # a real, reported crash the very first time the new --self-test step
    # ran for real in CI: FileNotFoundError on
    # Contents/Frameworks/transformers/models/__init__.pyc. The self-test
    # did exactly its job - caught a genuine packaging defect before it
    # ever reached a person, on the first real run.
    "transformers",
    "torch",
    "thinc",
    "srsly",
