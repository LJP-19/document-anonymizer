...
COLLECT_ALL = [
    ...
    # Two attempts at fixing its packaging through COLLECT_ALL/a custom
    # hook both failed identically on real CI (see
    # buildtools/hooks/hook-transformers.py's docstring for the full
    # history). transformers now gets its own dedicated hook using
    # PyInstaller's native module_collection_mode='pyz+py' mechanism
    # instead - --collect-all and a hook's own settings are two different,
    # possibly-conflicting paths to bundle the same package, and having
    # both active for a package that has needed three real attempts to
